...
!!$      end function compressed_index_fn
    ...
    function get_offset(iproc, nproc, n) result(is)
      ...
      if (nproc>1) then
          call mpigather(sendbuf=n_, recvbuf=narr, root=nproc-1, comm=bigdft_mpi%mpi_comm)
      else
          narr(0) = n_(1)
      end if
      ...
      if (nproc>1) then
          call mpiscatter(sendbuf=isarr, recvbuf=is_, root=nproc-1, comm=bigdft_mpi%mpi_comm)
      else
          is_(1) = isarr(0)
      end if
      is = is_(1)
