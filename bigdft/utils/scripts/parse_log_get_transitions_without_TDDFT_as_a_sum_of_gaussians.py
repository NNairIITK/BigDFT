...
e_min = min(e_diff) - 2.0 #minimal eneregy
e_max = max(e_diff) + 2.0 #maximal energy
...
DoE = [] #initialise 
for e_i in en:
  DoE.append(   np.sum(  np.exp( - (e_i - e_diff[:])**2 / (2 * sigma**2) )  )   ) #Append data corresponding to each energy grid
##print DoE
...
if write_output:
    ...
    myDoEfile_e = open('transitions_noTDDFT_'+str(args.nvirt).strip()+'v_gauss.dat','w')
