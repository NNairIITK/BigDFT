...
module parallel_linalg
  ...
  contains
    ...
    subroutine dsyev_parallel(iproc, nproc, blocksize, comm, jobz, uplo, n, a, lda, w, info)
      ...
      real(mp),parameter :: scale_factor=1000.0_mp
      ...
      blocksize_if: if (blocksize<0) then
          ...
          processIf: if(irow/=-1) then
              ...
              repeat_loop: do icl=1,2
                  ...
                  !write(*,*) 'before pdsyevx, iproc', iproc
                  call f_memcpy(src=la, dest=la_tmp)
                  ! Scale the matrix a little bit to increase the spread between the eigenvalues.
                  ! Otherwise there might be problems with the orthogonalization...
                  do icol=1,lncol
                      do irow=1,lnrow
                          la_tmp(irow,icol) = scale_factor*la(irow,icol)
                      end do
                  end do
                  call pdsyevx(jobz, 'a', 'l', n, la_tmp(1,1), 1, 1, desc_la, &
                               ...
                               -1.d0, lz(1,1), 1, 1, desc_lz, work, lwork, iwork, liwork, &
                               ifail, icluster, gap, info)
                  !write(*,*) 'after pdsyevx, iproc', iproc
                  if(info==0) then
                      ! Everything ok
                      exit repeat_loop
              ...
              !write(*,*) 'before pdelset2, iproc', iproc
              do i=1,n
                  do j=1,n
                      call pdelset2(a(j,i), lz(1,1), j, i, desc_la, 0.d0)
                  end do
              end do
              !write(*,*) 'after pdelset2, iproc', iproc
