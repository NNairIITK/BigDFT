!{\src2tex{textfont=tt}}
!!****f* ABINIT/abi_bonds_lgth_angles
!! NAME
!! abi_bonds_lgth_angles
!!
!! FUNCTION
...
&  rprimd,typat,xred,znucl)
 ...
 use abi_interfaces_geometry, except_this_one => abi_bonds_lgth_angles
...
!Local variables-------------------------------
 character(len=2), parameter :: symbol_name(103)= (/  &
&   ' H','He',                                        &
&   'Li','Be',' B',' C',' N',' O',' F','Ne',          &
...
!Initialize the file
 write(message, '(a,a)' ) ' abi_bonds_lgth_angles : about to open file ',fnameabo_app_geo
...
!Set up a list of character identifiers for all atoms : iden(ia)
 ...
 do ia=1,natom
   ...
   ii=nint(znucl(typat(ia)));symbol=symbol_name(ii)
...
&     ' abi_bonds_lgth_angles : BUG -',ch10,&
&     '  abi_bonds_lgth_angles cannot handle more than 9999 atoms, while natom=',natom,ch10,&
...
!            Allows for a small numerical inaccuracy
             ...
             write(message, '(a,a)' )ch10,&
&             ' abi_bonds_lgth_angles : BUG - the evaluation of the angle is wrong. '
             call abi_wrtout(std_out,message,'COLL')
             call abi_leave_new('COLL')
           end if
         else
