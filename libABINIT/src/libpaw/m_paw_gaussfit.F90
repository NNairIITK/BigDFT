...
! if(pawrad%mesh_type==1) nr=msz*2 !lineargrid
! if(pawrad%mesh_type/=1) nr=int(msz*1.2) !log grid
! call pawrad_init(mesh_tmp,mesh_size=nr,mesh_type=pawrad%mesh_type,rstep=pawrad%rstep,lstep=pawrad%lstep)
!
!write(*,*)'Fit done from 0 to',r_fit
