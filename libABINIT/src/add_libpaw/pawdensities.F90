...
!!
!! PARENTS
!!      make_efg_onsite,pawdenpot,pawenergy3,poslifetime
!!
!! CHILDREN
...
!Various inits
 ...
 if (present(one_over_rad2)) then
   ...
   LIBPAW_POINTER_ALLOCATE(one_over_rad2_,(mesh_size))
...
!  Scale densities with 1/r**2 and compute rho1(r=0) and trho1(r=0)
   if (cplex==2)  then
     LIBPAW_ALLOCATE(aa,(5))
     LIBPAW_ALLOCATE(bb,(5))
   end if
...
!  -- Print out moments of densities (if requested)
   if (abs(pawprtvol)>=2.and.opt_print==1.and.opt_dens<2) then
     LIBPAW_ALLOCATE(aa,(cplex*mesh_size))
     LIBPAW_ALLOCATE(bb,(cplex*mesh_size))
     if (opt_dens==0) then
...
!  ----- End loop over spin components
 ...
 if (.not.present(one_over_rad2))  then
   LIBPAW_POINTER_DEALLOCATE(one_over_rad2_)
 end if
