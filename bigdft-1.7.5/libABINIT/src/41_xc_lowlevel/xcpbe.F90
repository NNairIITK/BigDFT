...
!scalars
 ...
 real(dp) :: ec1_b4,ec1_den,ec1_log,ec1_q0,ec1_q0p,ec1_q1,ec1_q1p,ec1_q1pp
 ...
 real(dp) :: mac_q0p,mac_q1,mac_q1ppp
 ...
 real(dp) :: d3ecrs_drhodndrho2,d3ecrs_drhoupdrho2
...
!            fab: third order derivatives of the XC part in the spin polarized case
             d2vxci(ipts,3*ispden-2) = -2._dp*dvxci(ipts,2*ispden-1)*rhomot*rhomot*rhomot/3._dp


!            mixed derivatives of the exchange energy with respect to rho of 
...
!          fab : INGREDIENTS NEEDED
           ...
           b2fa=14._dp/9._dp*( threefourth_divpi**(third))*(1._dp-zeta)/(rhotot**(7._dp/3._dp))
           b1fa=(1._dp-zeta)/rhotot
           c2fa=(((1._dp-zeta)*rhotot)**2)*(-3._dp*(1._dp/((rhotot)**4)))
           ...
           g1fa=-third*(threefourth_divpi**(third))*((rhotot)**(-4._dp/3._dp))
           g2fa=(-1._dp-zeta)/rhotot
           h2fa=(2._dp/3._dp)*((threefourth_divpi)**(third))*((-1._dp-(7._dp/3._dp)*zeta)/(rhotot**(7._dp/3._dp)))
           h1fa=-zeta/rhotot
           i2fa=-2._dp*zeta*1._dp/((rhotot)**2)-3*zeta*zeta*1._dp/((rhotot)**2)+1._dp/((rhotot**2))
           i1fa=(zeta*zeta-1._dp)/rhotot
           m2fa=(-2._dp/3._dp)*((threefourth_divpi)**(third))*((7._dp/3._dp)*(zeta+1._dp)/(rhotot**(7._dp/3._dp)))
           m1fa=(-1._dp-zeta)/rhotot
           n2fa=2._dp*(1._dp-zeta)/rhotot-(-1._dp-zeta)*(-1._dp-zeta)/(rhotot**2)
...
&           (2._dp*ec1_q0*(ec1_q1p**3)*(2._dp*ec1_q1+1._dp))*(ec1_den**3)+  &
&           (2._dp*ec1_q1*ec1_q0*(ec1_q1p**3))*(ec1_den**2)      
...
&           2._dp*mac_q0*(mac_q1p**3)*(2._dp*mac_q1+1._dp)*(mac_den**3)+  &
&           2._dp*mac_q1*mac_q0*(mac_q1p**3)*mac_den**2         
           ...
           d3ecrs_dzeta3=(24._dp*(zeta**2)*f_zeta+36._dp*(zeta**2)*fp_zeta+  &
&           12._dp*(zeta**3)*fpp_zeta+(zeta**4)*fppp_zeta)*gcrs+   &
&           fppp_zeta*(-macrs)*fsec_inv                                   


           d3ecrs_drs2dzeta=(4._dp*(zeta**3)*f_zeta+zeta4*fp_zeta)*(d2gcrs_drs2)+   &
&           fp_zeta*zeta4*(-d2macrs_drs2)*fsec_inv                        
...
!          Moreover, the first part of the following two terms (d3ecrs_drhoupdrho2 and d3ecrs_drhodndrho2)
!          (i.e. until the term 16._dp*pi*pi....included) is the part which remains even in the
!          non spin polarized limit


           d3ecrs_drhoupdrho2=(128._dp*pi*pi/243._dp)*(rs**7)*(decrs_drs)- &
&           (48._dp*pi*pi/243._dp)*(rs**8)*d2ecrs_drs2- &
&           (16._dp*pi*pi/243._dp)*(rs**9)*d3ecrs_drs3+ &
&           a2fa*((-8._dp/27._dp)*(rs**4)*d2ecrs_drsdzeta+ &
&           (4._dp*pi*pi/27._dp)*(rs**5)*d3ecrs_drs2dzeta)   
           ...
           d3ecrs_drhodndrho2=(128._dp*pi*pi/243._dp)*(rs**7)*(decrs_drs)- &
&           (48._dp*pi*pi/243._dp)*(rs**8)*d2ecrs_drs2- &
&           (16._dp*pi*pi/243._dp)*(rs**9)*d3ecrs_drs3+ &
&           g2fa*((-8._dp/27._dp)*(rs**4)*d2ecrs_drsdzeta+ &
&           (4._dp*pi*pi/27._dp)*(rs**5)*d3ecrs_drs2dzeta)    
...
&           n2fa*d2ecrs_dzeta2+n1fa*(g1fa*d3ecrs_dzeta2drs+g2fa*d3ecrs_dzeta3)
         ...
         if(option==1 .or. option==-1 .or. option==3)then
           ...
           if(order==3) then
!            third order derivatives of the spin polarized correlation energy
