...
subroutine davidson(iproc,nproc,n1i,n2i,n3i,in,at,cpmult,fpmult,radii_cf,&
  ...
  call orthon_p(iproc,nproc,orbsv%norb,comms%nvctr_par(iproc,1),lr%wfd%nvctr_c+7*lr%wfd%nvctr_f,v,1)
  ...
  do iorb=1,orbsv%norb ! temporary variables 
     e(iorb,1,ise)= dot(commsv%nvctr_par(iproc,1),v(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1,&
          hv(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1)          != <psi|H|psi> 
     e(iorb,2,ise)= nrm2(commsv%nvctr_par(iproc,1),v(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1)**2                    != <psi|psi> 
  end do
...
!end if
  ...
  do iter=1,in%itermax
     ...
     do iorb=1,orbsv%norb
         call axpy(commsv%nvctr_par(iproc,1),-e(iorb,1,1),v(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1,&
              g(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1)
         !gradient = hv-e*v
         e(iorb,2,ise)= nrm2(commsv%nvctr_par(iproc,1),g(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1)**2
     ...
     do iorb=1,orbsv%norb
        do jorb=iorb,orbsv%norb!or 1,nvirte 
           ind=1+commsv%nvctr_par(iproc,1)*(iorb-1)
           jnd=1+commsv%nvctr_par(iproc,1)*(jorb-1)
           hamovr(iorb,jorb,ish-1)=               dot(commsv%nvctr_par(iproc,1),v(ind),1,hv(jnd),1)
           hamovr(jorb,iorb+orbsv%norb,ish-1)=        dot(commsv%nvctr_par(iproc,1),g(ind),1,hv(jnd),1)
           !=hamovr(iorb+orbsv%norb,jorb,ish-1)=        dot(commsv%nvctr_par(iproc,1),g(ind),1,hv(jnd),1)
           hamovr(iorb,jorb+orbsv%norb,ish-1)=        dot(commsv%nvctr_par(iproc,1),v(ind),1,hg(jnd),1)
           hamovr(iorb+orbsv%norb,jorb+orbsv%norb,ish-1)= dot(commsv%nvctr_par(iproc,1),g(ind),1,hg(jnd),1)

           hamovr(iorb,jorb,ish)=               dot(commsv%nvctr_par(iproc,1),v(ind),1, v(jnd),1)
           hamovr(jorb,iorb+orbsv%norb,ish)=       dot(commsv%nvctr_par(iproc,1),g(ind),1, v(jnd),1)
           !=hamovr(iorb+orbsv%norb,jorb,ish)=        dot(commsv%nvctr_par(iproc,1),g(ind),1, v(jnd),1)
           hamovr(iorb,jorb+orbsv%norb,ish)=        dot(commsv%nvctr_par(iproc,1),v(ind),1, g(jnd),1)
           hamovr(iorb+orbsv%norb,jorb+orbsv%norb,ish)= dot(commsv%nvctr_par(iproc,1),g(ind),1, g(jnd),1)
        enddo
     enddo

     !Note: The previous data layout allowed level 3 BLAS
!    call DGEMM('T','N',nvirte,nvirte,commsv%nvctr_par(iproc,1),1.d0,v(1,1),commsv%nvctr_par(iproc,1),&
!         hv(1,1),commsv%nvctr_par(iproc,1),0.d0,hamovr(1,1,ish-1),n2virt)
!    call DSYRK('U','T',n2virt,commsv%nvctr_par(iproc,1),1.d0,v(1,1),commsv%nvctr_par(iproc,1),0.d0,hamovr(1,1,ish),n2virt)!upper
     ...
     do jorb=1,orbsv%norb! v to update
        call razero(commsv%nvctr_par(iproc,1),hv(1+commsv%nvctr_par(iproc,1)*(jorb-1)))
        do iorb=1,orbsv%norb ! sum over v and g
           tt=hamovr(iorb,jorb,1)
           call axpy(commsv%nvctr_par(iproc,1),tt,v(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1,&
                hv(1+commsv%nvctr_par(iproc,1)*(jorb-1)),1)
           tt=hamovr(iorb+orbsv%norb,jorb,1)
           call axpy(commsv%nvctr_par(iproc,1),tt,g(1+commsv%nvctr_par(iproc,1)*(iorb-1)),1,&
                hv(1+commsv%nvctr_par(iproc,1)*(jorb-1)),1)
        enddo
     enddo

     call dcopy(commsv%nvctr_par(iproc,1)*orbsv%norb,hv,1,v,1)

     !Note: The previous data layout allowed level 3 BLAS
     !call DGEMM('N','N',commsv%nvctr_par(iproc,1),nvirte,n2virt,1.d0,v(1,1),commsv%nvctr_par(iproc,1),hamovr(1,1,1),n2virt,0.d0,hv(1,1),commsv%nvctr_par(iproc,1))
     !    dimensions    =m      =n   =k          m,k        k,n                   m,n             
     !call DCOPY(commsv%nvctr_par(iproc,1)*nvirte,hv(1,1),1,v(1,1),1)
