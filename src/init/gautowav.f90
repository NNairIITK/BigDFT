...
subroutine check_gaussian_expansion(iproc,nproc,orbs,lr,hx,hy,hz,psi,G,coeffs)
  ...
  real(wp), dimension(wfd%nvctr_c+7*wfd%nvctr_f,orbs%norbp), intent(in) :: psi
  ...
  allocate(workpsi((wfd%nvctr_c+7*wfd%nvctr_f)*orbs%norbp+ndebug),stat=i_stat)
  call memocc(i_stat,workpsi,'workpsi',subname)

  call gaussians_to_wavelets(lr%geocode,iproc,orbs,lr%d,hx,hy,hz,&
