...
subroutine DiagHam(iproc,nproc,natsc,nspin,orbs,wfd,comms,&
  ...
  differentInputGuess: if(.not. input%directDiag) then
      if(iproc==0) write(*,'(1x,a)') 'Iterative diagonalization of Hamiltonian...'
      ...
      call inputguessParallel(iproc, nproc, orbs, norbsc_arr, hamovr, psi, psit, input, nspin, nspinor, npsidim, comms, natsc, ndim_hamovr, norbsc)

  else

      if(iproc==0) write(*,'(1x,a)') 'Direct diagonalization of Hamiltonian...'
...
subroutine inputguessParallel(iproc, nproc, orbs, norbscArr, hamovr, psi, psiGuessWavelet, input, nspin, nspinor, sizePsi, comms, natsc, ndim_hamovr, norbsc)
!
! Purpose:
! ========
...
! Local variables
integer:: i, j, iorb, jorb, ispin, ii, jj, kk, norbtot, norbtotPad, iter, ierr, itermax, ist, i_stat, i_all, nprocSub, ishift, jjorb
integer:: norbi, norbj, iat, imatrst, imatrst2, ihs, jiorb, norbij, ncplx, ncomp, jspin, ist2, ishift1, ishift2, norb, blocksize, lwork
integer:: blocksizeSmall, nprocSubu, nprocSubd, nspinSub, isp, kp, ikpt, ikptp, istpsi, istpsit, nvctrp, kkSave, iiSave, jproc, ist3, ikptp2, ikpt2
integer:: istpsiS, istpsitS, info, wholeGroup, newGroup, newComm, wholeGroupu, newGroupu, newCommu, wholeGroupd, newGroupd, newCommd
integer,dimension(:),allocatable:: sendcounts, sdispls, recvcounts, rdispls, norbpArr, norbtotpArr, kArr, kstArr, nkArr, norbArr
...
spinLoop: do isp=1,nspinSub
    ...
    if(input%norbpInguess>norb) then
        if(.not. simul) then
            if(iproc==0) write(*,'(5x,a,2(i0,a))') 'WARNING: You want each process to treat ',input%norbpInguess,' orbitals, whereas there &
                                                 are only ',norb,'.'
            if(iproc==0) write(*,'(7x,a,i0,a)') 'The value of input%norbpInguess is adjusted to ',norb, '.'
        else
            if(iproc==0) write(*,'(3x,a,2(i0,a))') 'WARNING: You want each process to treat ',input%norbpInguess,' orbitals, whereas there &
                                                 are only ',norb,'.'
    ...
    if(.not. warning) then
        if(.not.simul) then
            ...
            if(iproc==0 .and. nprocSub<nproc) write(*,'(7x,a,2(i0,a))') 'Processes from ',nprocSub,' to ',nproc,' will idle.'
        else
            ...
            if(iproc==0 .and. kkSave/=0) write(*,'(5x,a,5(i0,a))') 'up orbitals: Processes from 0 to ',kkSave-1,&
                ' treat ',iiSave+1,' orbitals, processes from ',kkSave,' to ',nprocSub-1,' treat ',iiSave,' orbitals.'
            if(iproc==0 .and. kkSave==0) write(*,'(5x,a,2(i0,a))') 'up orbitals: Processes from 0 to ',nprocSub-1,' treat ',iiSave,' orbitals.'
            ...
            if(iproc==0 .and. nprocSub<nproc) write(*,'(5x,a,2(i0,a))') 'Processes from ',nprocSubu+nprocSubd,' to ',nproc,' will idle.'
        end if
    else
        if(.not. simul) then
            if(iproc==0) write(*,'(5x,a,i0,a)') 'Generating input guess with ',nprocSub,' processes:'
    ...
    if(.not. simul) then
        if(iproc==0 .and. ispin==1) write(*,'(5x,a,i0,a)') 'up orbitals: padding the orbitals with ',norbtotPad-norbtot,' zeros.'
    else
        if(iproc==0 .and. ispin==1) write(*,'(3x,a,i0,a)') 'up orbitals: padding the orbitals with ',norbtotPad-norbtot,' zeros.'
    end if
    if(simul) then
            ! Send some information to root process to ensure that they are printed in the correct order.
    ...
    if(.not. simul) then
        if((simul .and. iproc==0) .or. (.not. simul .and. ispin==2 .and. iproc==0)) write(*,'(5x,a,i0,a)') 'down orbitals: &
            padding the orbitals with ',norbtotPad-norbtot,' zeros.'
    else
        if((simul .and. iproc==0) .or. (.not. simul .and. ispin==2 .and. iproc==0)) write(*,'(3x,a,i0,a)') 'down orbitals: &
            padding the orbitals with ',norbtotPad-norbtot,' zeros.'
    end if
    ...
    processIf: if(iproc<nprocSub .or. (simul .and. iproc<nprocSubu+nprocSubd)) then
        ...
        do ikpt=1,kp
            ...
            do iorb=1,norbtot
                call dcopy(norbtot*nspinor, hamovr(ii+(iorb-1)*norbtot*nspinor+1,ispin,1,kk), 1, HamPad(1,iorb,ispin,ikpt), 1)
                call dcopy(norbtot*nspinor, hamovr(ii+(iorb-1)*norbtot*nspinor+1,ispin,2,kk), 1, overlapPad(1,iorb,ispin,ikpt), 1)
                do j=norbtot*nspinor+1,norbtotPad*nspinor
        ...
        do ikpt=1,kp
            if(nspinor==1) then
                call dsymm('l', 'u', norbtotPad, nkArr(ikpt), 1.d0, overlapPad(1,1,ispin,ikpt), norbtotPad, psiGuessP(1,kstArr(ikpt),ispin), &
                    norbtotPad, 0.d0, overlapPsiGuessP(1,kstArr(ikpt),ispin), norbtotPad)
            else
                call zhemm('l', 'l', norbtotPad, nkArr(ikpt), (1.d0,0.d0), overlapPad(1,1,ispin,ikpt), norbtotPad, psiGuessP(1,kstArr(ikpt),ispin), &
        ...
        if(.not. simul .or. iproc<nprocSubu) then
            call orthonormalize(iproc, nprocSub, norbtotPad, norb, norbpArr(iproc), norbpArr(0), norbtotpArr(0), psiGuessP(1,1,ispin), &
                overlapPsiGuessP(1,1,ispin), newComm, input, simul, orbs, 0, nspinor, blocksize, blocksizeSmall)
        else
            call orthonormalize(iproc, nprocSub, norbtotPad, norb, norbpArr(iproc), norbpArr(nprocSubu), norbtotpArr(nprocSubu), &
                psiGuessP(1,1,ispin), overlapPsiGuessP(1,1,ispin), newComm, input, simul, orbs, nprocSubu, nspinor, blocksize, blocksizeSmall)
        end if
        ...
        if(.not. simul) then
            ...
            if(iproc==0) write(*,'(3x,a,$)') 'Iteratively determining eigenvectors... '
        end if
        ...
        mainLoop: do iter=1,itermax
            do ikpt=1,kp
                ...
                if(nspinor==1) then
                    call dsymm('l', 'u', norbtotPad, nkArr(ikpt), 1.d0, HamPad(1,1,ispin,ikpt), norbtotPad, psiGuessP(1,kstArr(ikpt),ispin), &
                        norbtotPad, 0.d0, gradient(1,kstArr(ikpt)), norbtotPad)
                    call dsymm('l', 'u', norbtotPad, nkArr(ikpt), 1.d0, overlapPad(1,1,ispin,ikpt), norbtotPad, psiGuessP(1,kstArr(ikpt),ispin), &
                        norbtotPad, 0.d0, overlapPsiGuessP(1,kstArr(ikpt),ispin), norbtotPad)
                else
                    call zhemm('l', 'l', norbtotPad, nkArr(ikpt), (1.d0,0.d0), HamPad(1,1,ispin,ikpt), norbtotPad, psiGuessP(1,kstArr(ikpt),ispin), &
        ...
        if(.not. success) then
            if(iproc==0) write(*,'(a,i0,a,es9.3)') 'WARNING: no convergence after ',itermax,' iterations. gradientMax=',gradientMax(1)
        end if
    ...
    if(.not. simul) then
        if(iproc==0) write(*,'(5x,a,$)') 'Transforming to wavelet basis... '
    else
        if(iproc==0) write(*,'(3x,a,$)') 'Transforming to wavelet basis... '
    end if
    ...
    do ikptp=1,orbs%nkptsp
        ...
        if(.not.simul) then
            ...
            if(nspinor==1) then
                ...
                ii=(ikpt-1)*norb/orbs%nkpts+1
                call zgemm('n', 'n', nvctrp, norb/orbs%nkpts, norbtot, (1.d0,0.d0), psi(ishift2+istpsi), nvctrp, psiGuess(1,ii,ispin), &
...
if(norbsc>0) then
    ...
    do ikpt=1,orbs%nkpts
        ...
        do ispin=1,nspin
            if(ispin==1) then ; norb=orbs%norbu-norbsc ; else ; norb=orbs%norbd-norbsc ; end if
...
subroutine orthonormalize(iproc, nproc, norbtot, norb, norbp, norbpArr, norbtotpArr, psi, overlapPsi, newComm, input, simul, &
...
! Local variables
integer:: i, j, iorb, iblock, jblock, ii, jj, ist, jst, iter, iter2, gcd, blocksizeSmall, norbtotp, ierr, i_stat, i_all, getBlocksize
...
if(nproc>1) then
    ...
    call timing(iproc, 'Input_comput', 'OF')
    call timing(iproc, 'Input_commun', 'ON')
    call mpi_alltoallv(psiW(1), sendcounts, sdispls, mpi_double_precision, psiWTrans(1), recvcounts, rdispls, mpi_double_precision, newComm, ierr)
    call mpi_alltoallv(overlapPsiW(1), sendcounts, sdispls, mpi_double_precision, overlapPsiWTrans(1), recvcounts, rdispls, mpi_double_precision, newComm, ierr)
    call timing(iproc, 'Input_commun', 'OF')
    call timing(iproc, 'Input_comput', 'ON')
else
...
do iblock=1,iter
    ...
    call choleskyOverlap(iproc, nproc, norbtotp, blocksize, psiWTrans(1), overlapPsiWTrans(1), newComm, ist, norb/orbs%nkpts, orbs%nkpts, nspinor)
end do
...
remainingIf: if(blocksize*iter/=norb/orbs%nkpts) then
    ...
    do iblock=1,iter2
        ...
        ! Orthonormalize the current bunch of vectors.
        call choleskyOverlap(iproc, nproc, norbtotp, blocksizeSmall, psiWTrans(1), overlapPsiWTrans(1), newComm, ist, norb/orbs%nkpts, orbs%nkpts, nspinor)
    end do
end if remainingIf
...
if(nproc>1) then
    ...
    call timing(iproc, 'Input_comput', 'OF')
    call timing(iproc, 'Input_commun', 'ON')
    call mpi_alltoallv(psiWTrans(1), sendcounts, sdispls, mpi_double_precision, psiW(1), recvcounts, rdispls, mpi_double_precision, newComm, ierr)
    call timing(iproc, 'Input_commun', 'OF')
    call timing(iproc, 'Input_comput', 'ON')
...
subroutine gramschmidtOverlap(iproc, nproc, norbtot, blocksize, psi, overlapPsi, newComm, istThis, istOther, norb, nkpts, nspinor)
!
! Purpose:
! =======
...
if(nproc>1) then ; ist=2 ; else ; ist=1 ; end if
...
do ikpt=1,nkpts
    if(nspinor==1) then
        call dgemm('t', 'n', blocksize, blocksize, norbtot, 1.d0, psi(1,istOther,ikpt), norbtot, overlapPsi(1,istThis,ikpt), norbtot, 0.d0, ovrlp(1,ikpt,ist), blocksize)
    else
        call zgemm('c', 'n', blocksize, blocksize, norbtot, (1.d0,0.d0), psi(1,istOther,ikpt), norbtot, overlapPsi(1,istThis,ikpt), norbtot, (0.d0,0.d0), ovrlp(1,ikpt,ist), blocksize)
    end if
end do
if(nproc>1) then
    call timing(iproc, 'Input_comput', 'OF')
    call timing(iproc, 'Input_commun', 'ON')
    call mpi_allreduce (ovrlp(1,1,2), ovrlp(1,1,1), blocksize*blocksize*nspinor*nkpts, mpi_double_precision, mpi_sum, newComm, ierr)
    call timing(iproc, 'Input_commun', 'OF')
    call timing(iproc, 'Input_comput', 'ON')
end if
...
do ikpt=1,nkpts
    ...
    if(nspinor==1) then
        call dgemm('n', 'n', norbtot, blocksize, blocksize, -1.d0, psi(1,istOther,ikpt), norbtot, ovrlp(1,ikpt,1), blocksize, 0.d0, A, norbtot)
    else
        call zgemm('n', 'n', norbtot, blocksize, blocksize, (-1.d0,0.d0), psi(1,istOther,ikpt), norbtot, ovrlp(1,ikpt,1), blocksize, (0.d0,0.d0), A, norbtot)
    end if
...
subroutine choleskyOverlap(iproc, nproc, norbtot, blocksize, psi, overlapPsi, newComm, istart, norb, nkpts, nspinor)
!
! Purpose:
! =======
...
do ikpt=1,nkpts
    if(nspinor==1) then
        call dgemm('t', 'n', blocksize, blocksize, norbtot, 1.d0, psi(1,istart,ikpt), norbtot, overlapPsi(1,istart,ikpt), norbtot, 0.d0, ovrlp(1,ikpt,ist), blocksize)
    else
        call zgemm('c', 'n', blocksize, blocksize, norbtot, (1.d0,0.d0), psi(1,istart,ikpt), norbtot, overlapPsi(1,istart,ikpt), norbtot, (0.d0,0.d0), ovrlp(1,ikpt,ist), blocksize)
    end if
end do
if(nproc>1) then
    call timing(iproc, 'Input_comput', 'OF')
    call timing(iproc, 'Input_commun', 'ON')
    call mpi_allreduce (ovrlp(1,1,2), ovrlp(1,1,1), blocksize*blocksize*nkpts*nspinor, mpi_double_precision, mpi_sum, newComm, ierr)
    call timing(iproc, 'Input_commun', 'OF')
    call timing(iproc, 'Input_comput', 'ON')
end if
...
do ikpt=1,nkpts
    ...
    if(nspinor==1) then
        call dtrmm('r', 'u', 'n', 'n', norbtot, blocksize, 1.d0, ovrlp(1,ikpt,1), blocksize, psi(1,istart,ikpt), norbtot)
    else
        call ztrmm('r', 'l', 'c', 'n', norbtot, blocksize, (1.d0,0.d0), ovrlp(1,ikpt,1), blocksize, psi(1,istart,ikpt), norbtot)
    end if
end do
