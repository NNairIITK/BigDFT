...
!!$  end do chk_nlcc
  ...
  if (at%donlcc) then
     ...
     !initalise it 
     if (n3d.gt.0) call to_zero(d%n1i*d%n2i*n3d*10,rhocore(1,1,1,1))
...
subroutine atomic_occupation_numbers(filename,ityp,nspin,at,nmax,lmax,nelecmax,neleconf,nsccode,mxpl,mxchg)
  ...
  do iat=1,at%astruct%nat
     ...
     if (at%astruct%iatype(iat) == ityp) then
        ...
        call charge_and_spol(at%astruct%input_polarization(iat),ichg,ispol)
        if (found) then
           ...
           if (abs(ispol) > mxpl+abs(ichg)) then
              ...
              write(*,'(1x,a,i0,a,a,2(a,i0))')&
                   ...
                   ' (',trim(at%astruct%atomnames(ityp)),') must be <=',mxpl,&
                   ', while found ',ispol
              stop 
           end if
           if (abs(ichg) > mxchg) then
              ...
              write(*,'(1x,a,i0,a,a,2(a,i0))')&
                   ...
                   ' (',trim(at%astruct%atomnames(ityp)),') must be <=',mxchg,&
                   ', while found ',ichg
              stop
           end if
