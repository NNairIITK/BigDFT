subroutine NL_HGH_application(npspcode,psppar,ncplx_p,n_p,wfd_p,proj,&
  ...
  call proj_dot_psi(n_p*ncplx_p,wfd_p,proj,n_w*ncplx_w,wfd_w,psi,psi_mask,psi_pack,scpr)
  ...
  call scpr_proj_p_hpsi(n_p*ncplx_p,wfd_p,proj,n_w*ncplx_w,wfd_w,psi_mask,psi_pack,scpr,hpsi)
...
subroutine proj_dot_psi(n_p,wfd_p,proj,n_w,wfd_w,psi,psi_mask,psi_pack,scpr)
  ...
  real(wp), dimension(wfd_w%nvctr_c+7*wfd_w%nvctr_f,n_w), intent(in) :: psi !< components of wavefunction
  integer, dimension(3,wfd_w%nseg_c+wfd_w%nseg_f), intent(inout) :: psi_mask !<lookup array in the wfn segments
  ...
  !calculate the scalar product with the first projector.
  !also the compressed wavefunction can be calculated if the number of projectors is bigger than one
  call wpdot_keys_pack(wfd_w%nvctr_c,wfd_w%nvctr_f,wfd_w%nseg_c,wfd_w%nseg_f,&
       ...
       psi_mask(1,1),psi_mask(1,is_sw),&
       psi_pack(1,1),psi_pack(is_p,1),scpr(1,1))
  !use now mask arrays to end the creation of the packed psi
  do iw=2,n_w
     call wpdot_keys_pack(wfd_w%nvctr_c,wfd_w%nvctr_f,wfd_w%nseg_c,wfd_w%nseg_f,&
          wfd_w%keyvglob(1),wfd_w%keyvglob(is_sw),wfd_w%keyglob(1,1),wfd_w%keyglob(1,is_sw),&
          psi(1,iw),psi(is_w,iw),&
...
!!$     call wpdot_mask_pack(wfd_w%nvctr_c,wfd_w%nvctr_f,wfd_w%nseg_c,wfd_w%nseg_f,&
!!$          psi_mask(1,1),psi_mask(1,is_sw),psi(1,iw),psi(is_w,iw),&
...
subroutine scpr_proj_p_hpsi(n_p,wfd_p,proj,n_w,wfd_w,&
  ...
  real(wp), dimension(wfd_p%nvctr_c+7*wfd_p%nvctr_f,n_p), intent(in) :: proj !< components of the projectors
  integer, dimension(3,wfd_w%nseg_c+wfd_w%nseg_f), intent(in) :: psi_mask !<lookup array in the wfn segments
  !indicating the points where data have to be taken for dot product
  ! always produced. Has to be initialized to zero first
...
!!$     call waxpy_mask_unpack(wfd_w%nvctr_c,wfd_w%nvctr_f,wfd_w%nseg_c,wfd_w%nseg_f,&
!!$          psi_mask(1,1),psi_mask(1,is_sw),hpsi_pack(1,iw),hpsi_pack(is_p,iw),&
!!$          hpsi(1,iw),hpsi(is_w,iw),&
...
subroutine wpdot_keys(  &
     ...
     amask_c,amask_f,scpr)
  use module_base, only: wp
  implicit none

  real(wp), dimension(mavctr_c), intent(in) :: apsi_c
  real(wp), dimension(7,mavctr_f), intent(in) :: apsi_f
  integer, dimension(3,maseg_c), intent(inout) :: amask_c
  integer, dimension(3,maseg_f), intent(inout) :: amask_f
...
!!$  real(wp), intent(out) :: scpr
!!$  !local variables
!!$  integer :: ibseg,jaj,jb1,jb0,jbj,iaoff,iboff,length,i,ja0,ja1,j
...
!!$  !$omp private(i,jaj,iaoff,length,ja1,ja0,jb1,jb0,iboff,tt,j,scpr0,scpr1) &
...
!!$           scpr0=scpr0+tt*bpsi_c(jbj+iboff+i)
!!$        enddo
!!$
!!$        !call dot_c_inline(length,jaj+iaoff,jbj+iboff,scpr0)
!!$
...
!!$              scpr1=scpr1+tt*bpsi_f(j,jbj+iboff+i)
!!$           end do
!!$        enddo
!!$
!!$        !call dot_f_inline(length,jaj+iaoff,jbj+iboff,scpr1)
!!$
...
subroutine wpdot_keys_pack(  &
     mavctr_c,mavctr_f,maseg_c,maseg_f,keyav_c,keyav_f,keyag_c,keyag_f,apsi_c,apsi_f,  &
     mbvctr_c,mbvctr_f,mbseg_c,mbseg_f,keybv_c,keybv_f,keybg_c,keybg_f,bpsi_c,bpsi_f,&
     amask_c,amask_f,apack_c,apack_f,scpr)
  use module_base, only: wp
  implicit none
  real(wp), dimension(mavctr_c), intent(in) :: apsi_c
  real(wp), dimension(7,mavctr_f), intent(in) :: apsi_f
...
!!$  !$omp shared (apsi_f,scpr,apack_c,apack_f) &
  !$omp parallel default(shared) &
  !$omp private(jaj,iaoff,length,ja1,ja0,jb1,jb0,iboff) &
  !$omp private(jbj,ibseg,iaseg0)!!!,ithread,nthread,ibsegs,ibsege,nchunk)
