...
module parameters 
  ...
  ! Fedwa El-Mellouhi February 2006
...
MODULE SWpotential
  ...
  real(8),dimension(:,:),allocatable :: SW_parameters
  integer,dimension(:), allocatable ::    angles_to_fit !angles to fit per atom
  integer,dimension(:,:,:),allocatable :: jk_of_angle
  real(8),dimension(:,:), allocatable ::  optimum_angle
  integer      ::                         len_vect_to_fit
  integer      ::                         configs_to_fit
  
  ! 1   : real(8), parameter :: SIGMA = 2.095037426D0
  ! 2   : real(8), parameter :: A =  7.049556277D0
  ! 3   : real(8), parameter :: ALPHA = 1.8d0 
  ! 4   : real(8), parameter :: BETA = 0.60222455844D0
  !  real(8), parameter :: EPSILON = 2.16823D0* 1.07d0 !  FE Balamane et al. PRB 46, 2250
  ! 5   : real(8), parameter :: EPSILON = 2.16823D0 ! Original SW
  ! 6  :  integer, parameter :: P = 4
  ! 7  :  real(8), parameter :: GAMMA = 1.2d0
  ! 8  :  real(8), parameter :: LAMBDA = (21.0d0 * 1.0d0)
  ! 9 :    real(8), parameter :: ONE_THIRD = 1.0d0/3.0d0
  ...
  real(8), dimension(2,2) :: A0, S0

  real(8), allocatable,dimension(:,:) ::force_ref_fit  !!reference force for fit
  real(8), allocatable,dimension(:,:) ::force_work_fit  !!working forces for fit
  real(8), allocatable,dimension(:,:) :: pos_ref_fit  !!reference position for fit
...
subroutine fit_SW_potential()
  ...
  integer :: i,j,k,l,param

  real(8) :: praxis
  real(8) , dimension(:),allocatable :: vector_to_fit
  
  real(8) :: trash_energy
  real(8),dimension(3) :: invbox
  real(8), parameter :: tolerance = 0.40d0
  real(8) :: step_size = 0.05d0
  integer,parameter :: max_mc_steps = 20
  real(8) :: inv_temperature = 3.0d0
  integer :: ierror
  real(8) :: value,dummy
  real(8) :: diff_square_force
  real(8), dimension(0:nproc-1,0:nproc-1) :: sendbuff
  real(8),dimension(0:nproc-1,0:nproc-1) :: recvbuff
  real(8),dimension(3*natoms)     :: box_vec
  real(8),dimension(natoms) :: diff_vector
...
subroutine mc_fit(vec_size,vector,tolerance,current_value,numnei,nei)
  ...
  real(8), dimension(vec_size), intent(inout) :: vector
  ...
  integer :: i,j,k,l,param,my_counter,this_atom
  real(8) :: diff_square_force
  real(8) :: diff_square_force_one

  real(8) , dimension(vec_size) :: tmp_vector
...
subroutine fit_sd(vec_size,vector,tolerance,current_value,numnei,nei)
  ...
  real(8) :: difference
  real(8), dimension(vec_size) :: tmp_vector,grad_vector
  real(8) :: step
  integer :: iter,param,my_counter,this_atom,i
  integer, parameter :: MAX_ITER = 50
  real(8), parameter :: STEPSIZE = 1.0D0  ! Size of displacement
...
function diff_square_force_one(P,numnei,nei,this_atom)
  ...
  real(8) :: diff_square_force_one
  real(8),dimension(len_vect_to_fit),intent(in) :: P
  integer,dimension(natoms),intent(in) :: numnei
  integer,dimension(natoms,maxnei),intent(in) :: nei
...
subroutine deriv_diff_square_force(P,DF,numnei,nei)
  ...
  real(8),dimension(len_vect_to_fit),intent(in) :: P
  real(8), dimension(len_vect_to_fit),intent(out) :: DF
  
  integer,dimension(natoms),intent(in) :: numnei
  integer,dimension(natoms,maxnei),intent(in) :: nei

  real(8),dimension(len_vect_to_fit) ::P_before,P_after
  real(8) :: diff_square_force_one
  real(8) :: valeur1,valeur2
...
subroutine SWcalczone(nat,posa,boxl,tmp_force, this_atom,numnei,nei)
  ...
  real(8), dimension(:), pointer :: fxa, fya, fza


  integer :: i,j, i_id, j_id, ind_j, k, k_id, ind_k,l,ind_l
  integer :: my_counter_i,ierror
  real(8) :: invsig
  ...
  real(8) :: xik,yik,zik,rik, rik2, invrik,rhoik
  logical :: go_on
