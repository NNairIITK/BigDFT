...
subroutine create_bayes_data()
  ...
  real(kind=8),dimension(:),allocatable :: coord_x,coord_y_coord_z,func_to_send
...
subroutine bayes_learn_from_data(what_direction)
  ...
  do i = 1,data_size
     do j = i,data_size
           sqrd_total = 1.0d0
           call calc_kernel(data_coord_x(i),data_coord_x(j),delta,sigma(what_direction,1),sqrd_exp)
           sqrd_total =sqrd_total * sqrd_exp
           call calc_kernel(data_coord_y(i),data_coord_y(j),delta,sigma(what_direction,2),sqrd_exp)
           sqrd_total =sqrd_total * sqrd_exp
           call calc_kernel(data_coord_z(i),data_coord_z(j),delta,sigma(what_direction,3),sqrd_exp)
           sqrd_total =sqrd_total * sqrd_exp
           call calc_kernel(data_coord_depth(i),data_coord_depth(j),delta,sigma(what_direction,4),sqrd_exp)
...
subroutine bayes_calc_one_force(coord_in,coord_nei,what_direction,force_out,variance)
  ...
  do i = 1,data_size
     sqrd_total = 1.0d0
     call calc_kernel(coord_in(1),data_coord_x(i),delta,sigma(what_direction,1),sqrd_exp)
     sqrd_total =sqrd_total * sqrd_exp
     call calc_kernel(coord_in(2),data_coord_y(i),delta,sigma(what_direction,2),sqrd_exp)
     sqrd_total =sqrd_total * sqrd_exp
     call calc_kernel(coord_in(3),data_coord_z(i),delta,sigma(what_direction,3),sqrd_exp)
     sqrd_total =sqrd_total * sqrd_exp
     call calc_kernel(coord_in(4),data_coord_depth(i),delta,sigma(what_direction,4),sqrd_exp)
     sqrd_total =sqrd_total * sqrd_exp*delta*delta
...
!> ART calc_kernel
subroutine calc_kernel(x1,x2,delta,sigma,kernel) !this subroutine calculates the kernel
  implicit none
  real(kind=8), intent(in) :: x1,x2,delta,sigma
...
subroutine calcforce_bayes(nat,posa,boxl,forca, energy)
  ...
  real(kind=8) :: force_bayes,variancex,variancey,variancez,variance_totale
  integer,dimension(nat,maxnei) :: nei
  integer,dimension(nat) :: numnei

  real(kind=8) :: lowest_error,ran3,delta_tmp,tmp_delta
  ...
  input_bayes(1) = forca(i)
  input_bayes(2) = forca(i+nat)
  input_bayes(3) = forca(i+nat+nat)
  input_bayes(4) = posa(i+nat+nat)
