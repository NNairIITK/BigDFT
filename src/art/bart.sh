...
setenv Activation_MaxIter             400   # Maximum number of iteraction for reaching the saddle point
...
setenv Basin_Factor                   1.6   # Factor multiplying Increment_Size for leaving the basin
...
setenv Eigenvalue_Threshold          -1.3   # Eigenvalue threshold for leaving basin
...
setenv calc_of_projection          .True.   # if it is true; it calculates the projection only at every two steps 
...
setenv Number_Lanczos_Vectors_A        16   # Number of vectors included in lanczos procedure
setenv Number_Lanczos_Vectors_C        15   # Number of vectors included in lanczos procedure
setenv delta_disp_Lanczos            0.02   # Step of the numerical derivative of forces in lanczos (Ang)
...
#setenv delta_threshold                0.2   # if delta_e < delta_thr .and. delr < delr_thr, in the
...
setenv REFCONFIG                refconfig   # Reference configuration
