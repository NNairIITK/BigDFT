...
setenv gnrm        1.5E-05
setenv Inflection  4 
setenv NATOMS    216                         # Number of atoms in the problem
setenv type1       N 
setenv type2       B 
setenv type3       C
...
setenv Temperature                       1.0   # Temperature in kcal/mol, if negative always reject the event
setenv Max_Number_Events                1000   # Maximum number of events
setenv Type_of_Events             list_local   # Initial move for events - global or local
...
setenv Activation_MaxIter                500   # Maximum number of iteraction for reaching the saddle point
...
setenv Basin_Factor                      2.0   # Factor multiplying Increment_Size for leaving the basin
...
setenv Eigenvalue_Threshold             -1.9   # Eigenvalue threshold for leaving basin
setenv Max_Iter_Basin                     10   # Maximum number of iteraction for leaving the basin (kter)
...
setenv Number_Lanczos_Vectors_A           15   # Number of vectors included in lanczos procedure
setenv Number_Lanczos_Vectors_C           14   # Number of vectors included in lanczos procedure
setenv delta_disp_Lanczos              0.010   # Step of the numerical derivative of forces in lanczos (Ang)
...
setenv Exit_Force_Threshold              0.2   # Threshold for convergence at saddle point
...
setenv delta_threshold                  0.15   # if delta_e < delta_thr .and. delr < delr_thr, in the
setenv delr_threshold                    0.5   # convergence stage, then we kill the event
...
setenv Write_JMOL                     .True.   # Writes the configuration in jmol format.
###### RUN THE SIMULATION #######################################################################
