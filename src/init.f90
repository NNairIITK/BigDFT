...
subroutine getLocalizedBasis2(iproc, nproc, at, orbs, Glr, input, orbsLIN, commsLIN, rxyz, &
    nspin, nlpspd, proj, nscatterarr, ngatherarr, rhopot, GPU, pkernelseq, phi, hphi, trH, rxyzParabola)
!
! Purpose:
! ========
!   Calculates the localized basis functions phi. These basis functions are eigenfunctions of the ordinary Hamiltonian
...
real(8),dimension(:),allocatable:: phir, eval, work
real(8),dimension(:,:),allocatable:: HamSmall
real(8):: hxh, hyh, hzh, dis, tt, rr, trace, ddot, dnrm2, fnrm, fnrmMax, meanAlpha, gnrm, gnrm_zero, kx, ky, kz, tt1, tt2, tt3
type(workarr_sumrho) :: w
type(workarr_locham) :: w2
!integer,dimension(:),allocatable:: onWhichAtom
real(8),dimension(:),pointer:: phiWorkPointer
real(8),dimension(:),allocatable:: phiWorkPointer2
logical:: debug, precond
logical,dimension(:),allocatable:: move
character(len=*),parameter:: subname='getLocalizedBasis'
real(8),dimension(:),allocatable:: phiTemp

allocate(phiTemp(size(hphi)))
...
allocate(hphiold(orbsLIN%npsidim), stat=istat)
call memocc(istat, orbsLIN%npsidim, 'hphiold', subname)
!allocate(alpha(orbsLIN%norbp), stat=istat)
allocate(alpha(orbsLIN%norb), stat=istat)
call memocc(istat, orbsLIN%norbp, 'alpha', subname)
...
! Orthonormalize the orbitals (probably not needed here, but just to be sure)
allocate(phiWorkPointer(size(phi)), stat=istat)
call transpose_v(iproc, nproc, orbsLIN, Glr%wfd, commsLIN, phi, work=phiWorkPointer)
call orthogonalize(iproc, nproc, orbsLIN, commsLIN, Glr%wfd, phi, input)
call untranspose_v(iproc, nproc, orbsLIN, Glr%wfd, commsLIN, phi, work=phiWorkPointer)
...
iterLoop: do it=1,itMax
    ...
    ! Calculate constrained gradient. This subroutine also calculates the trace trH.
    allocate(phiWorkPointer(size(phi)), stat=istat)
    call transpose_v(iproc, nproc, orbsLIN, Glr%wfd, commsLIN, hphi, work=phiWorkPointer)
    call transpose_v(iproc, nproc, orbsLIN, Glr%wfd, commsLIN, phi, work=phiWorkPointer)
    call orthoconstraintNotSymmetric(iproc, nproc, orbsLIN, commsLIN, Glr%wfd, phi, hphi, trH)

    ! Calculate the norm of the gradient and determone the angle between the current gradient and that
    ! of the previous iteration.
    nvctrp=commsLIN%nvctr_par(iproc,1) ! 1 for k-point
    istart=1
    orbLoop2: do iorb=1,orbsLIN%norb
       
        if(it>1) then
            tt1=ddot(nvctrp*orbs%nspinor, hphi(istart), 1, hphiold(istart), 1)
            tt=tt1
            call mpi_allreduce(tt, tt1, 1, mpi_double_precision, mpi_sum, mpi_comm_world, ierr)
        end if
        tt2=ddot(nvctrp*orbs%nspinor, hphi(istart), 1, hphi(istart), 1)
        tt=tt2
        call mpi_allreduce(tt, tt2, 1, mpi_double_precision, mpi_sum, mpi_comm_world, ierr)
        if(it>1) then 
            tt3=ddot(nvctrp*orbs%nspinor, hphiold(istart), 1, hphiold(istart), 1)
            tt=tt3
            call mpi_allreduce(tt, tt3, 1, mpi_double_precision, mpi_sum, mpi_comm_world, ierr)
        end if
        if(it>1) then
        ! Adapt step size for the steepest descent minimization.
            tt=tt1/sqrt(tt2*tt3)
        ...
        ! Calculate the norm of the constrained gradient.
        !tt=ddot((Glr%wfd%nvctr_c+7*Glr%wfd%nvctr_f)*orbs%nspinor, hphi(istart), 1, hphi(istart), 1)
        fnrm=fnrm+tt2
        if(tt>fnrmMax) then
            fnrmMax=tt2
        end if
    ...
    ncong=5
    gnrm=1.d3 ; gnrm_zero=1.d3
    if(precond) call preconditionallLIN(iproc, nproc, orbsLIN, Glr, input%hx, input%hy, input%hz, ncong,&
         hphi, gnrm, gnrm_zero, fnrmMax, at%nat, rxyz, orbsLIN%onWhichAtom, at, it)
    ...
    tt=sum(alpha)
    tt=tt/dble(orbs%nbasisp)
    call mpi_allreduce(tt, meanAlpha, 1, mpi_double_precision, mpi_sum, mpi_comm_world, ierr)
    meanAlpha=meanAlpha/dble(nproc)

    if(iproc==0) write(*,'(a,i6,3es15.7,2f21.7,es12.4)') 'iter, gnrm, fnrm, fnrmMax, trace, trH, meanAlpha', &
        it, gnrm, fnrm, fnrmMax, trace, trH, meanAlpha
    if(iproc==0) write(1000,'(i6,2es15.7,f21.7,es12.4)') it, fnrm, fnrmMax, trH, meanAlpha
    !if(fnrmMax<1.d-2) then
    if(fnrmMax<1.d0 .and. it>=2) then
        if(iproc==0) write(*,'(a,i0,a)') 'converged in ', it, ' iterations.'
        call plotOrbitals(iproc, orbsLIN, Glr, phi, at%nat, rxyz, orbsLIN%onWhichAtom, hxh, hyh, hzh, 1)
        exit iterLoop
    end if
    if(fnrmMax<1.d2 .and. .not.precond) then
        if(iproc==0) write(*,'(a)') 'starting preconditioner...'
