...
!  real(wp), dimension(5), parameter :: c4 = (/0.07142857_wp,-0.57142857_wp,1.92857143_wp,-3.42857143_wp,3.00000000_wp/)
  ...
  if(iproc==0)print *, "NNdbg: ***** history is called *****"
...
!dbg
     if(iproc==0)print *, "NNdbg: oldpsis(",wfn_history+1,") to oldpsis(",istep,")"
     if(iproc==0)print *, "NNdbg: ....checking","oldpsis(",istep,")"
...
!dbg 
     ...
     if (istep_history == 0) then
        if(iproc==0)print *, "NNdbg: istep_history ==0; copying oldpsis(1..to wfn_history+1)"
...
!
 do jstep=0,wfn_history
     call reformatmywaves(iproc,orbs,atoms,&
          ...
          oldpsis(jstep)%rxyz,oldpsis(jstep)%Lzd%Glr%wfd,&
     ...
     if(iproc.eq.0)print *, "NNdbg reformat is done for oldpsis(",jstep,")"
!dbg
...
!dbg
     ...
     if(iproc.eq.0)print *, "NNdbg; overlap is computed for <Psi(",jstep,")|Psi(",wfn_history+1,")>"
...
!     call myoverlap(iproc,nproc,Lzd,orbs,comms,psi(1,1),psi(1,1))
     if(iproc.eq.0)print *, "NNdbg wavefunction Psi(",jstep,") is rotated and added to Psi"
 end do

  if(iproc.eq.0)print *, "NNdbg Normalizing the final orbital"
