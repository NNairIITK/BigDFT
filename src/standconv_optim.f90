...
! wavelet part
 ...
 ! + (1/2) d^2/dz^2
    ...
    return
         ...
         subroutine ConvolkineticPPwrong(nbuf,nb1,nb2,nb3,n1,n2,n3, &
...
! (1/2) d^2/dx^2
    do i3=0,n3
    do i2=0,n2
        do i1=ibyz_c(1,i2,i3)+nbuf,ibyz_c(2,i2,i3)+nbuf-1,2
                t111=0.d0 ; s111=0.d0
                u111=0.d0 ; v111=0.d0
                do ii=max(i1+lowfil,ibbyz_c(1,i2+nbuf,i3+nbuf)),min(i1+1+lupfil,ibbyz_c(2,i2+nbuf,i3+nbuf))
                    t111=t111 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*a(ii-i1)
                    s111=s111 + x(ii,2,i2+nbuf,1,i3+nbuf,1)*b(ii-i1)
                    u111=u111 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*a(ii-i1-1)
                    v111=v111 + x(ii,2,i2+nbuf,1,i3+nbuf,1)*b(ii-i1-1)
                enddo
                ekin1=ekin1+(t111+s111)*x(i1  ,1,i2+nbuf,1,i3+nbuf,1)
                ekin2=ekin2+(u111+v111)*x(i1+1,1,i2+nbuf,1,i3+nbuf,1)
        enddo
        do i1=i1,ibyz_c(2,i2,i3)+nbuf  ! remainder of unrolled loop
                t111=0.d0 ; s111=0.d0
        ...
        do i1=ibyz_f(1,i2,i3)+nbuf,ibyz_f(2,i2,i3)+nbuf-3,4
                t211=0.d0 ; s211=0.d0 ; u211=0.d0 ; v211=0.d0 
                do ii=max(i1+lowfil,ibbyz_c(1,i2+nbuf,i3+nbuf)),min(i1+3+lupfil,ibbyz_c(2,i2+nbuf,i3+nbuf))
                    t211=t211 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*c(ii-i1)
                    s211=s211 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*c(ii-i1-1)
                    u211=u211 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*c(ii-i1-2)
                    v211=v211 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*c(ii-i1-3)
                enddo
                ekin1=ekin1+t211*x(i1  ,2,i2+nbuf,1,i3+nbuf,1)
                ekin2=ekin2+s211*x(i1+1,2,i2+nbuf,1,i3+nbuf,1)
                ekin3=ekin3+u211*x(i1+2,2,i2+nbuf,1,i3+nbuf,1)
                ekin4=ekin4+v211*x(i1+3,2,i2+nbuf,1,i3+nbuf,1)
        enddo
        do i1=i1,ibyz_f(2,i2,i3)+nbuf ! remainder of unrolled loop
                t211=0.d0 
                do ii=max(i1+lowfil,ibbyz_c(1,i2+nbuf,i3+nbuf)),min(i1+lupfil,ibbyz_c(2,i2+nbuf,i3+nbuf))
                    t211=t211 + x(ii,1,i2+nbuf,1,i3+nbuf,1)*c(ii-i1)
                enddo
                ekin5=ekin5+t211*x(i1,2,i2+nbuf,1,i3+nbuf,1)
        enddo
    enddo
    enddo
...
! + (1/2) d^2/dy^2
    ...
    do ii2=0,n2,nb
    do i1=0,n1
        if (ii2.ge.ibxz_c(1,i1,i3)+nbuf .and. ii2+(nb-1).le.ibxz_c(2,i1,i3)+nbuf) then
        do i2=ii2,ii2+(nb-1)-1,2 ! middle
                t111=0.d0 ; s111=0.d0
                u111=0.d0 ; v111=0.d0
                do ii=max(i2+lowfil,ibbxz_c(1,i1+nbuf,i3+nbuf)),min(i2+1+lupfil,ibbxz_c(2,i1+nbuf,i3+nbuf))
                    t111=t111 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*a(ii-i2)
                    s111=s111 + x(i1+nbuf,1,ii,2,i3+nbuf,1)*b(ii-i2)
                    u111=u111 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*a(ii-i2-1)
                    v111=v111 + x(i1+nbuf,1,ii,2,i3+nbuf,1)*b(ii-i2-1)
                enddo
                ekin1=ekin1+(t111+s111)*x(i1+nbuf,1,i2  ,1,i3+nbuf,1)
                ekin2=ekin2+(u111+v111)*x(i1+nbuf,1,i2+1,1,i3+nbuf,1)
        enddo
        else
        do i2=max(ii2,ibxz_c(1,i1,i3)+nbuf),min(ii2+(nb-1),ibxz_c(2,i1,i3)+nbuf) ! head and tail 
                t111=0.d0 ; s111=0.d0
                do ii=max(i2+lowfil,ibbxz_c(1,i1+nbuf,i3+nbuf)),min(i2+lupfil,ibbxz_c(2,i1+nbuf,i3+nbuf))
                    t111=t111 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*a(ii-i2)
                    s111=s111 + x(i1+nbuf,1,ii,2,i3+nbuf,1)*b(ii-i2)
                enddo
                ekin3=ekin3+(t111+s111)*x(i1+nbuf,1,i2,1,i3+nbuf,1)
        enddo
        endif

        if (ii2.ge.ibxz_f(1,i1,i3)+nbuf .and. ii2+(nb-1).le.ibxz_f(2,i1,i3)+nbuf) then 
        do i2=ii2,ii2+(nb-1)-3,4 ! middle
                t121=0.d0 ; s121=0.d0 ; u121=0.d0 ; v121=0.d0 
                do ii=max(i2+lowfil,ibbxz_c(1,i1+nbuf,i3+nbuf)),min(i2+3+lupfil,ibbxz_c(2,i1+nbuf,i3+nbuf))
                    t121=t121 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*c(ii-i2)
                    s121=s121 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*c(ii-i2-1)
                    u121=u121 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*c(ii-i2-2)
                    v121=v121 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*c(ii-i2-3)
                enddo
                ekin1=ekin1+t121*x(i1+nbuf,1,i2  ,2,i3+nbuf,1)
                ekin2=ekin2+s121*x(i1+nbuf,1,i2+1,2,i3+nbuf,1)
                ekin3=ekin3+u121*x(i1+nbuf,1,i2+2,2,i3+nbuf,1)
                ekin4=ekin4+v121*x(i1+nbuf,1,i2+3,2,i3+nbuf,1)
        enddo
        else
        do i2=max(ii2,ibxz_f(1,i1,i3)+nbuf),min(ii2+(nb-1),ibxz_f(2,i1,i3)+nbuf) ! head and tail 
                t121=0.d0 
                do ii=max(i2+lowfil,ibbxz_c(1,i1+nbuf,i3+nbuf)),min(i2+lupfil,ibbxz_c(2,i1+nbuf,i3+nbuf))
                    t121=t121 + x(i1+nbuf,1,ii,1,i3+nbuf,1)*c(ii-i2)
                enddo
...
! + (1/2) d^2/dz^2
    ...
    do ii3=0,n3,nb
    do i1=0,n1
        if (ii3.ge.ibxy_c(1,i1,i2)+nbuf .and. ii3+(nb-1).le.ibxy_c(2,i1,i2)+nbuf) then
        do i3=ii3,ii3+(nb-1)-1,2 ! middle
                t111=0.d0 ; s111=0.d0
                u111=0.d0 ; v111=0.d0
                do ii=max(i3+lowfil,ibbxy_c(1,i1+nbuf,i2+nbuf)),min(i3+1+lupfil,ibbxy_c(2,i1+nbuf,i2+nbuf))
                    t111=t111 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*a(ii-i3)
                    s111=s111 + x(i1+nbuf,1,i2+nbuf,1,ii,2)*b(ii-i3)
                    u111=u111 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*a(ii-i3-1)
                    v111=v111 + x(i1+nbuf,1,i2+nbuf,1,ii,2)*b(ii-i3-1)
                enddo
                ekin1=ekin1+(t111+s111)*x(i1+nbuf,1,i2+nbuf,1,i3  ,1)
                ekin2=ekin2+(u111+v111)*x(i1+nbuf,1,i2+nbuf,1,i3+1,1)
        enddo
        else
        do i3=max(ii3,ibxy_c(1,i1,i2)+nbuf),min(ii3+(nb-1),ibxy_c(2,i1,i2)+nbuf) ! head and tail 
                t111=0.d0 ; s111=0.d0
        ...
        if (ii3.ge.ibxy_f(1,i1,i2)+nbuf .and. ii3+(nb-1).le.ibxy_f(2,i1,i2)+nbuf) then
        do i3=ii3,ii3+(nb-1)-3,4 ! middle
                t112=0.d0 ; s112=0.d0 ; u112=0.d0 ; v112=0.d0 
                do ii=max(i3+lowfil,ibbxy_c(1,i1+nbuf,i2+nbuf)),min(i3+3+lupfil,ibbxy_c(2,i1+nbuf,i2+nbuf))
                    t112=t112 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*c(ii-i3)
                    s112=s112 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*c(ii-i3-1)
                    u112=u112 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*c(ii-i3-2)
                    v112=v112 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*c(ii-i3-3)
                enddo
                ekin1=ekin1+t112*x(i1+nbuf,1,i2+nbuf,1,i3  ,2)
                ekin2=ekin2+s112*x(i1+nbuf,1,i2+nbuf,1,i3+1,2)
                ekin3=ekin3+u112*x(i1+nbuf,1,i2+nbuf,1,i3+2,2)
                ekin4=ekin4+v112*x(i1+nbuf,1,i2+nbuf,1,i3+3,2)
        enddo
        else
        do i3=max(ii3,ibxy_f(1,i1,i2)+nbuf),min(ii3+(nb-1),ibxy_f(2,i1,i2)+nbuf) ! head and tail 
                t112=0.d0 
                do ii=max(i3+lowfil,ibbxy_c(1,i1+nbuf,i2+nbuf)),min(i3+lupfil,ibbxy_c(2,i1+nbuf,i2+nbuf))
                    t112=t112 + x(i1+nbuf,1,i2+nbuf,1,ii,1)*c(ii-i3)
                enddo
...
! wavelet part
 ...
 ! + (1/2) d^2/dy^2
    ...
    do ii2=nfl2,nfu2,nb
    do i1=nfl1,nfu1
       do i2=max(ii2,ibxz_f(1,i1,i3))+nbuf,min(ii2+(nb-1),ibxz_f(2,i1,i3))+nbuf
                t112=0.d0;t121=0.d0;t122=0.d0;t212=0.d0;t221=0.d0;t222=0.d0;t211=0.d0 
 ...
 ! + (1/2) d^2/dz^2
    ...
    do ii3=nfl3,nfu3,nb
    do i1=nfl1,nfu1
       do i3=max(ii3,ibxy_f(1,i1,i2))+nbuf,min(ii3+(nb-1),ibxy_f(2,i1,i2))+nbuf
                t112=0.d0;t121=0.d0;t122=0.d0;t212=0.d0;t221=0.d0;t222=0.d0;t211=0.d0 
