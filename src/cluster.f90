...
subroutine cluster(nproc,iproc,atoms,rxyz,energy,fxyz,&
  ...
  integer :: nvirt
  integer :: nelec,ndegree_ip,nvctrp,mids,iorb,ids,idiistol,j,norbpeff
...
!!$        call gaussian_orthogonality(iproc,nproc,norb,norbp,gbd,gaucoeffs)
  ...
  call nonlocal_forces(iproc,n1,n2,n3,hx,hy,hz,cpmult,fpmult,atoms,rxyz,radii_cf,&
       orbs,nlpspd,proj,wfd,psi,gxyz,calc_tail) !refill projectors for tails
  ...
  if (calc_tail .and. atoms%geocode == 'F') then
     ...
     call CalculateTailCorrection(iproc,nproc,atoms,n1,n2,n3,rbuf,orbs,&
          nfl1,nfu1,nfl2,nfu2,nfl3,nfu3,wfd,nlpspd,ncongt,&
          pot,hx,rxyz,radii_cf,crmult,frmult,cpmult,fpmult,in%nspin,&
