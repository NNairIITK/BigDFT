...
! New variables
real(8),dimension(:),allocatable:: phi, phiPulay
type(orbitals_data):: orbsLIN
type(communications_arrays):: commsLIN
logical:: writeBasis
real(8),dimension(:,:),allocatable:: rxyzParab, rxyzParabOld, fxyzPulay
integer:: icomp, ishift, ishell, sizeRhopot, iforce, niter, DIISHist, nvctrp, istart, infoBasisFunctions
real(8):: energyLIN, energym2, energym1, energy0, energyp1, energyp2, pulayForce, pulayShift, pulayShift0, ddot, dnrm2
real(8):: convCritInit, beta, ebs_mod, ebs_mod2
real(8),dimension(:),allocatable:: rhopotCorrect, alphaArr, cosangle
real(8),dimension(:,:),allocatable:: occupForInguess, fxyzOld
character(len=50):: outputFile, ch
...
real(8),dimension(:,:,:),allocatable:: rxyzParabolaHist
real(8),dimension(:),pointer:: phiWorkPointer
  ...
  case(100)
     ! linear scaling
     !write(*,*) 'LINEAR SCALING VERSION'
     !call improveOrbitals(iproc, nproc, nspin, Glr, orbs, orbsLIN, commsLIN, atoms, rxyz, nscatterarr, ngatherarr, nlpspd, proj, rhopot, GPU, in, pkernelseq, phi)
     ...
     allocate(occupForInguess(32,atoms%nat))
     !occupForInguess=0.d0
     !do iat=1,atoms%nat
     !    ishell=0
     !    do
     !        ishell=ishell+1
     !        if(atoms%aocc(ishell,iat)==0.d0) exit
     !        occupForInguess(ishell,iat)=atoms%aocc(ishell,iat)
     ...
     !     nlpspd,proj,pkernel,pkernelseq,ixc,phi,hpsi,psit,Gvirt,&
     !     nscatterarr,ngatherarr,nspin,0,atoms%symObj,irrzon,phnons,GPU,in)
 !write(100+iproc,*) phi
     if (nvirt > norbv) then
        nvirt = norbv
     ...
     !call initializeLocRegLIN(iproc, nproc, Glr, orbsLIN, atoms, in, rxyz, radii_cf)
  case(1)
               ...
               if(inputpsi==100)then
                    ...
                    forceLoop: do iforce=1,15
                        ...
                        call improveOrbitals(iproc, nproc, nspin, Glr, orbs, orbsLIN, comms, commsLIN, atoms, rxyz, & 
                         ...
                         psi, psit, iter, infoBasisFunctions, ebs_mod=ebs_mod)
                            ...
                            do icomp=1,3
                                do ishift=-2,2
                                    ...
                                    call improveOrbitals(iproc, nproc, nspin, Glr, orbs, orbsLIN, comms, commsLIN, &
                                         ...
                                         rhopot, GPU, in, pkernelseq, phiPulay, psi, psit, -1, infoBasisFunctions, &
