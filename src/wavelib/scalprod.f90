...
subroutine wpdot_wrap1(ncplx,mavctr_c,mavctr_f,maseg_c,maseg_f,keyav,keyag,apsi,  &
     mbvctr_c,mbvctr_f,mbseg_c,mbseg_f,keybv,keybg,bpsi,scpr,iaseg0_c,iaseg0_f, &
  ...
  integer, dimension(mbseg_c), intent(in) :: iaseg0_c
  integer, dimension(mbseg_f), intent(in) :: iaseg0_f
  integer, dimension(maseg_c), intent(in) :: keyag_c_lin!linear version of second inidces of keyag_c
  integer, dimension(maseg_f), intent(in) :: keyag_f_lin!linear version of second inidces of keyag_f
  !local variables
  ...
  do ia=1,ncplx
     do ib=1,ncplx
        call wpdot1(mavctr_c,mavctr_f,maseg_c,maseg_f,&
             ...
             scalprod(ia,ib),iaseg0_c,iaseg0_f)
     end do
  end do
...
subroutine wpdot1(  &
     mavctr_c,mavctr_f,maseg_c,maseg_f,keyav_c,keyav_f,keyag_c_lin,keyag_f_lin,apsi_c,apsi_f,  &
     mbvctr_c,mbvctr_f,mbseg_c,mbseg_f,keybv_c,keybv_f,keybg_c,keybg_f,bpsi_c,bpsi_f,scpr,iaseg0_c,iaseg0_f)
  use module_base
  implicit none
...
!$omp do schedule(static)
   do ibseg=1,mbseg_c
     ...
     !call hunt1(keyag_c_lin,maseg_c,keybg_c(1,ibseg),iaseg0(ibseg))
     
     jaj=keyav_c(iaseg0_c(ibseg))
     length = jb1-jb0
     iaoff = jb0-keyag_c_lin(iaseg0_c(ibseg))!jb0-ja0
        do i=0,length
...
!$omp do schedule(static)
   do ibseg=1,mbseg_f
     ...
     !call hunt1(keyag_f_lin,maseg_f,keybg_f(1,ibseg),iaseg0_f(ibseg))
     
     jaj=keyav_f(iaseg0_f(ibseg))
     length = jb1-jb0
     iaoff = jb0-keyag_f_lin(iaseg0_f(ibseg))
     do i=0,length
