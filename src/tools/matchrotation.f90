...
program matchrotation
    ...
    real(8)::cell(3),rcm(3),rcminit(3),x(6),f(6),evec(6,6),eval(6),direction(6)
    real(8)::tt1,tt2,tt3,tt4,tt5,tt6
    real(8)::dis,dis_sq,dx,dy,dz,dis_sq_min,fnrm,alpha,dis_sq_old,tcpuinit
    ...
    do icycle=1,10**6
        ...
        dis=sqrt(dis)
        !---------------------------------------------------------------------------
        if(icycle>1) call reindex(nat,rat1,rat2,an)
        !---------------------------------------------------------------------------
        iproc=0
        !x(1:6)=0.d0
...
!*****************************************************************************************
subroutine reindex(nat,rat1,rat2,an)
    implicit none
    integer::nat,iat,itry,mat1,mat2
    real(8)::rat1(3,nat),rat2(3,nat),tt,dx,dy,dz,dis,disold,dis_init,xyz(1:3)
...
subroutine writepathway(nat,rat1,rat2,an,cell,filename)
    implicit none
    integer::nat,iat,ip
    real(8)::rat1(3,nat),rat2(3,nat),cell(3),dt,t
    character(5)::an(nat)
    character(*)::filename
...
subroutine readpositionatoms(nat,filename,cell,rat,an)
    implicit none
    integer::iproc,nat,iat,nat_tmp
    character(*)::filename
    real(8)::cell(3),rat(3,nat)
    character(5)::an(nat)
    character(40)::units
    open(unit=9,file=filename,status='old')
...
subroutine transform(x,nat,rat)
    implicit none
    integer::nat,iat
    real(8)::x(6),rat(3,nat),xat1,yat1,zat1
    real(8)::u(3,3),ud(3,3,3),udd(3,3,3,3)
...
subroutine calenergyforces(nat,rat1,rat2,x,chi,f,evec,eval)
    implicit none
    integer::nat,i,j,iat,info
    real(8)::rat1(3,nat),rat2(3,nat),chi,x(6),f(6),evec(6,6),eval(6)
    real(8)::u(3,3),ud(3,3,3),udd(3,3,3,3),work(500)
    real(8)::tt1,tt2,tt3,tt4,xat1,yat1,zat1,xat2,yat2,zat2
    real(8)::a(3),b1(3),b2(3),b3(3),c11(3),c22(3),c33(3),c12(3),c13(3),c23(3)
    call buildorthogonal(x,u,ud,udd)
    chi=0.d0
    f(1:6)=0.d0
    evec(1:6,1:6)=0.d0
...
subroutine buildorthogonal(x,u,ud,udd)
    real(8)::x(3),u(3,3),ud(3,3,3),udd(3,3,3,3)
    real(8)::cx1,cx2,cx3,sx1,sx2,sx3
    !-------------------------------------------------------------
...
subroutine checkwhetherCPUtimeexceeded(tcpuinit,cputimeexceeded)
    implicit none
    logical::cputimeexceeded
    real(8)::cpulimit,tcpuinit,tcpucurr
    integer::k
