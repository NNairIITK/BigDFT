...
subroutine bfgs(nproc,iproc,x,f,epot,at,rst,in,ncount_bigdft,fail)
    ...
    logical::check,switched,autoprecon,onlysih,fail
    !-------------------------------------------
    integer::n,nr,iconf,istat,m,it,nwork,i,j,n_silicon,iter_old,infocode,iprecon,iwrite,nitsd,histlen,nfluct
    real(gp) :: fluctsum
    real(8)::alat(3),ddot,shift,tmp,fnormmax_sw,limbfgs,ehist(30),erel
    real(8), allocatable::diag(:),work(:),xt(:),xtc(:),xc(:),ft(:),ftc(:),fc(:),hess(:,:),eval(:),hesst(:,:),evalt(:),xdft(:),xwrite(:)
    character*4 fn4
    character*40 comment
    type(parameterminimization)::parmin
    !Read from input.geopt
    ...
    !-------------------------------------------------------------------------------------
        ...
        allocate(hess(nr,nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating hess.'
        allocate(eval(nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating eval.'
        allocate(hesst(nr,nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating hesst.'
        allocate(evalt(nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating evalt.'
        allocate(xt(nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating xt.'
        allocate(xtc(nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating xtc.'
        allocate(xc(n),stat=istat);if(istat/=0) stop 'ERROR: failure allocating xc.'
        allocate(ft(nr),stat=istat);if(istat/=0) stop 'ERROR: failure allocating ft.'
        allocate(diag(n),stat=istat);if(istat/=0) stop 'ERROR: failure allocating diag.'
        allocate(xdft(n),stat=istat);if(istat/=0) stop 'ERROR: failure allocating xdft.'
        allocate(xwrite(n),stat=istat);if(istat/=0) stop 'ERROR: failure allocating xwrite.'
        allocate(work(n*(2*m+1)+2*m),stat=istat)
        ...
        call fnrmandforcemax(f,tmp,fmax,at,in,rst)
...
1020        if((ncount_bigdft==0 .or. parmin%iflag==2 .or. switched) .and. parmin%DIAGCO) then
...
!                if (iproc==0) call wtposout(????ncount_bigdft+500,epot,x,at)
                ...
                call fnrmandforcemax(f,fnrm,fmax,at,in,rst)
                call convcheck(fnrm,fmax,fluct*in%frac_fluct,in%forcemax,check)
...
! If user realizes the BFGS does not work efficiently, a switch to CG can be obtained by putting SDCG in the input.geopt file
   ...
   enddo   ! main BFGS loop

        deallocate(hess,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating hess.'
        deallocate(eval,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating eval.'
        deallocate(hesst,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating hesst.'
        deallocate(evalt,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating evalt.'
        deallocate(xt,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating xt.'
        deallocate(xtc,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating xtc.'
        deallocate(xc,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating xc.'
        ...
        deallocate(xdft,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating xdft.'
        deallocate(xwrite,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating xwrite.'
...
!        allocate(tpos(3*nat),grad(3*nat),pos(3*nat))
...
!calculate the max of the forces
 call fnrmandforcemax(fxyz,tmp,fmax,at,in,rst)
...
do iat=1,at%nat
 ...
 call fnrmandforcemax(fxyz,fnrm,fmax,at,in,rst)
if (iproc.eq.0) then
...
type(atoms_data), intent(inout) :: at,nfluct
type(input_variables), intent(inout) :: in
type(restart_objects), intent(inout) :: rst
integer, intent(inout) :: ncount_bigdft
real(gp), dimension(3,at%nat), intent(inout) :: rxyz
...
logical :: care, check
integer :: nsatur,iat,itot,itsd,i_stat,i_all,infocode
...
loop_sd: do
 ...
 call fnrmandforcemax(ff,fnrm,fmax,at,in,rst)
...
subroutine fnrmandforcemax(ff,fnrm,fmax,at,in,rst)
    use module_base
    use module_types
    use module_interfaces
    implicit none
