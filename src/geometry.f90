subroutine conjgrad(nproc,iproc,at,rxyz,etot,fxyz, &
  ...
  if (in%betax <= 0.d0) then
     call detbetax(nproc,iproc,at,rxyz,&
  ...
  call steepdes(nproc,iproc,at,rxyz,etot,fxyz,&
  ...
  if (fnrm < sqrt(1.d0*at%nat)*(fluct+flucto+fluctoo)*in%frac_fluct/3.d0 .or. &
     ...
     loop_cg: do
        ...
        if (iproc.eq.0) then
           ...
           write(*,'(1x,a,1pe14.5,2(1x,a,1pe14.5))')&
                ...
                'fluct=',sqrt(real(at%nat,kind=8))*(fluct+flucto+fluctoo)*in%frac_fluct/3.d0
        end if
  ...
  subroutine steepdes(nproc,iproc,at,rxyz,etot,ff,&
    ...
    redo_sd: do
       ...
       loop_sd: do
          ...
          in%output_grid=.false.
          in%output_wf=.false.
          call call_cluster(nproc,iproc,at,rxyz,etot,ff,&
               psi,wfd,norbp,norb,eval,n1,n2,n3,rxyz_old,in,infocode)
          ...
          if (care .and. itsd >= 3 .and. beta == in%betax .and. &
               ...
               de1 > -.1d0 .and. de1 < anoise .and. &            !energy slowly decreasing
               fnrm <= .1d0 .and. fnrm/fnrmitm1 > .8d0 .and. &   !norm of forces is saturating
               de2 > -2.d0*anoise .and. df2 > -2.d0*anoise) then !close to a local minimum (E&F)
             nsatur=nsatur+1
          else
             nsatur=0
          endif
