...
subroutine timing(iproc,category,action)
  ...
  if (action.eq.'IN') then  ! INIT
     ...
     call sum_results(parallel,iproc,ncat,cats,ittime,itsum,timesum)
...
subroutine sum_results(parallel,iproc,ncat,cats,ittime,itsum,timesum)
  implicit none
  include 'mpif.h'
  logical, intent(in) :: parallel
  integer, intent(in) :: iproc,ncat,ittime
  ...
  timesum(ncat+1)=real(total,kind=8)
  if (iend-ittime < 0) then
     timesum(ncat+1)=real(iend-ittime+count_max,kind=8)/real(count_rate,kind=8)
  else
     timesum(ncat+1)=real(iend-ittime,kind=8)/real(count_rate,kind=8)
  end if
  sum=0.d0
  do i=1,ncat
...
!!$     enddo
  ...
  if (iproc.eq.0) then
     ...
     do i=1,ncat
        ...
        write(60,'(a14,1(10x,1pe9.2),10x,0pf8.1 )') cats(i),timetot(i)/real(nproc,kind=8),pc
        total_pc=total_pc+pc
     enddo
     write(60,'(70("-"))')
