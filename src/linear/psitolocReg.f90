...
!! SOURCE:
!!
subroutine overlap_matrix_between_locreg(ilr,ilr2,isovrlp,nlr,nspin,psidim1,psishift1,npsidim,orbsi,Glr,Llr,Lpsi,&
           Lhpsi,Localnorb,outofzone,dim_Lhamovr,Lhamovr)
 ...
 implicit none
  ...
  ! Local Variables
  !########################################
  integer :: iolr,i_all,i_stat,psidim2,psishift2
  integer :: ldim,ldim1,ldim2
...
!         if (nvctrp == 0) cycle
       ...
       call local_overlap_matrices((Localnorb(ilr)+Localnorb(ilr2))*nspin,Localnorb(ilr),&
          Localnorb(ilr2),ldim,nspin,orbs%nspinor,dim_Lhamovr,Ahamovr(1,1,1),Lopsi(1),Lohpsi(1))  
!      ispsi=ispsi+nvctrp*norbtot*orbs%nspinor
!      end do
...
subroutine local_overlap_matrices(norbe,norb1,norb2,nvctrp,nspin,nspinor,ndim_hamovr,hamovr,psi,hpsi)
  use module_base
  implicit none
  ...
  !local variables
  integer :: iorbst,iorbst2,imatrst,ispin,ncomp,ncplx
  ...
  do ispin=1,nspin !this construct assumes that the semicore is identical for both the spins
     if (nspinor ==1) then
        ...
        call gemm('T','N',norb1,norb2,nvctrp,1.0_wp,psi(1,iorbst),max(1,nvctrp),&
             psi(1,iorbst2),max(1,nvctrp),0.0_wp,hamovr(imatrst,2),norb1)
     else
        ...
        call c_gemm('C','N',norb2,norb1,ncomp*nvctrp,(1.0_wp,0.0_wp),psi(1,iorbst),&
             max(1,ncomp*nvctrp),psi(1,iorbst2),max(1,ncomp*nvctrp),&
             (0.0_wp,0.0_wp),hamovr(imatrst,2),norb1)
     end if
     iorbst =iorbst + norb1
     iorbst2=iorbst2+ norb2
...
!write(900,'(a,8i9,es15.7)') 'lmin, lmax, Gmin, Gmax, icheck, offset, Glr%wfd%keyv(isegG), ix, lpsi(icheck) ', &
!        lmin, lmax, Gmin, Gmax, icheck, offset, Glr%wfd%keyv(isegG), ix, lpsi(icheck) 
           !!! loop over the orbitals
           !!do iorbs=1,orbs%norbp*orbs%nspinor
           !!   lpsi(icheck+lincrement*(iorbs-1))=psi(Glr%wfd%keyv(isegG)+offset+ix+Gincrement*(iorbs-1))
           !!end do
...
! Could optimize the routine by looping only on Gsegs not looped on before (TO DO)
     do isegG = Glr%wfd%nseg_c+1,Glr%wfd%nseg_c+Glr%wfd%nseg_f
        ...
        do ix = 0,length
           icheck = icheck + 1
           do igrid=1,7
              !lpsi(start+icheck+igrid*lfinc) = gpsi(Gstart+Glr%wfd%keyv(isegG)+offset+ix+igrid*Gfinc)
!write(*,'(a,2i12)') 'ind l, ind g', start+(icheck-1)*7+igrid, Gstart+(Glr%wfd%keyv(isegG)+ix-1)*7+offset+igrid
