...
subroutine build_ks_orbitals(iproc, nproc, tmb, KSwfn, at, rxyz, denspot, GPU, &
  ...
  call get_coeff(iproc, nproc, LINEAR_MIXDENS_SIMPLE, KSwfn%orbs, at, rxyz, denspot, GPU, infoCoeff, &
       ...
       input%lin%order_taylor,input%purification_quickreturn,input%adjust_FOE_temperature,input%calculate_KS_residue)
  ...
  call get_coeff(iproc, nproc, LINEAR_MIXDENS_SIMPLE, KSwfn%orbs, at, rxyz, denspot, GPU, infoCoeff, &
       ...
       input%calculate_KS_residue, updatekernel=.false.)
  energy=energs%ebs-energs%eh+energs%exc-energs%evxc-energs%eexctX+energs%eion+energs%edisp
  energyDiff=energy-energyold
  energyold=energy
