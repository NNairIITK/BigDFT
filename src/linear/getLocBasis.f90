...
real(gp) :: econf
  ...
  iterLoop: do
      ...
      call check_cutoff(iproc, nproc, tmb%orbs, tmb%lzd, tmb%lzd%hgrids(1), tmb%lzd%hgrids(2), tmb%lzd%hgrids(3), 5.d0, tmb%confdatarr, tmb%psi)
...
!!!END DEBUG
      ...
      call hpsitopsi_linear(iproc, nproc, it, ldiis, tmb, tmblarge, &
           lhphi, lphiold, alpha, trH, meanAlpha, alpha_max, alphaDIIS, psidiff)
      overlap_calculated=.false.
...
integer, dimension(3) :: ishift !temporary variable in view of wavefunction creation
  ...
  do iorb=1,orbs%norbp
     ...
     !write(*,'(a,2i8,4es16.7)') 'iproc, iorb, confdatarr(iorb)%rxyzConf, locrad', iproc, iorb, confdatarr(iorb)%rxyzConf, locrad
