...
subroutine purify_kernel(iproc, nproc, tmb, overlap_calculated, it_shift, it_opt, order_taylor, &
  ...
  if (iproc==0) call yaml_sequence_open('purification process')
      ...
      do it=1,it_opt
          ...
          if (nproc > 1) then
              call mpiallred(ks, mpi_sum, bigdft_mpi%mpi_comm)
          end if
...
subroutine get_KS_residue(iproc, nproc, tmb, KSorbs, hpsit_c, hpsit_f, KSres)
  ...
  if (nproc > 1) then
      call mpiallred(KH, mpi_sum, bigdft_mpi%mpi_comm)
  end if
  ...
  if (nproc > 1) then
      call mpiallred(KHKH, mpi_sum, bigdft_mpi%mpi_comm)
  end if
  call f_free(KH)
  ...
  if (nproc > 1) then
      call mpiallred(Kgrad, mpi_sum, bigdft_mpi%mpi_comm)
  end if
