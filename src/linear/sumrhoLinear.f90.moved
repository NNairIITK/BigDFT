...
subroutine sumrhoLinear(iproc,nproc,nlr,orbs,Glr,Llr,ixc,hxh,hyh,hzh,psi,rho,nrho,&
      nscatterarr,nspin,GPU,symObj,irrzon,phnons,norbPsi,coeff)
  ...
  implicit none
  !Arguments
  integer, intent(in) :: iproc,nproc,nlr,nrho,nspin,ixc,symObj, norbPsi
  ...
  real(8),dimension(orbs%norb,norbPsi),intent(in):: coeff
  !Local variables
  character(len=*), parameter :: subname='sumrho'
  logical :: rsflag
  integer :: nrhotot,n3d,itmred
  integer :: nspinn
  integer :: i1,i2,i3,i3off,i3s,i,ispin,jproc,i_all,i_stat,ierr,j3,j3p,j, ilr
  real(dp) :: charge,tt
...
do ilr=1,nlr
    if(iproc==0) write(*,'(x,a,i0)') 'in sumrho: >>>>>>> zone ', ilr
    if(iproc==0) write(*,'(3x,a,4i10)') 'nseg_c, nseg_f, nvctr_c, nvctr_f', Llr(ilr)%wfd%nseg_c, Llr(ilr)%wfd%nseg_f, Llr(ilr)%wfd%nvctr_c, Llr(ilr)%wfd%nvctr_f
    if(iproc==0) write(*,'(3x,a,3i8)') 'Llr(ilr)%d%n1i, Llr(ilr)%d%n2i, Llr(ilr)%d%n3i', Llr(ilr)%d%n1i, Llr(ilr)%d%n2i, Llr(ilr)%d%n3i
end do
...
!!$  end if
  ...
  if (GPUconv) then
     ...
     !!   stop 'not implemented in sumrhoLinear'
     ...
     !!   ! Set the 10^-40 since the array will be squared later.
     !!   call tenminusten(max(nrho,1)*nspin,rho,nproc)
     !!end if
...
subroutine local_partial_densityLinear(iproc,nproc,nlr,rsflag,nscatterarr,&
  ...
  integer, dimension(0:nproc-1,4), intent(in) :: nscatterarr !n3d,n3p,i3s+i3xcsh-1,i3xcsh
  real(wp), dimension(orbs%npsidim), intent(in) :: psi
  real(8),dimension(orbs%norb,norbPsi):: coeff
  !local variables
  character(len=*), parameter :: subname='local_partial_density'
  integer :: iorb,i_stat,i_all,ii, ind, indSmall, indLarge, jorb
  integer :: oidx,sidx,nspinn,npsir,ncomplex, i1, i2, i3, ilr, ispin
  real(gp) :: hfac,spinval
  type(workarr_sumrho) :: w
  real(wp), dimension(:,:), allocatable :: psir
  real(dp), dimension(:),allocatable :: rho_p, coeffPerOrb
real(8):: dnrm2
 ...
 !initialize rho
  if (libxc_functionals_isgga()) then
      ...
      !call tenminustwenty(max(nrho,1)*nspin,rho,nproc)
      ! Initialize to 10^-10, since the array will be squared later.
      call tenminusten(max(nrho,1)*nspin,rho,nproc)
  end if

  allocate(coeffPerOrb(norbPsi), stat=i_stat)
  call memocc(i_stat,coeffPerOrb,'coeffPerOrb',subname)
  ...
  if(iproc==0) write(*,'(3x,a,3i8)') 'Llr(ilr)%d%n1i, Llr(ilr)%d%n2i, Llr(ilr)%d%n3i', Llr(ilr)%d%n1i, Llr(ilr)%d%n2i, Llr(ilr)%d%n3i
      ...
      orbitalsLoop: do iorb=1,orbs%norbp
         ...
         if (libxc_functionals_isgga()) then
             ...
             ! rho has already been set to 10^-20 (actually 10^-40, since it will be quared later),
             ! so set rho_p to zero, since it sums up the parts from rho_p.
         ...
         if (hfac /= 0.d0) then
            ...
            do oidx=0,ncomplex
               ...
               case('F')
                  do jorb=1,norbPsi
                      coeffPerOrb(jorb)=coeff(iorb,jorb)
                      write(*,'(x,a,3i,es20.12)') 'iproc, iorb, jorb, coeffPerOrb(jorb)', iproc, iorb, jorb, coeffPerOrb(jorb)
                  end do
                  call partial_density_linear(rsflag,nproc,Llr(ilr)%d%n1i,Llr(ilr)%d%n2i,Llr(ilr)%d%n3i,&
                       npsir,nspinn,nrhotot,&
                       hfac,nscatterarr,spinval,psir,rho_p,orbs%norb,norbPsi,coeffPerOrb,Llr(ilr)%bounds%ibyyzz_r)
                      ...
                      do i3=1,Llr(ilr)%d%n3i
                          do i2=1,Llr(ilr)%d%n2i
                              do i1=1,Llr(ilr)%d%n1i
                                  ...
                                  if(indLarge>size(rho,1)) then
                                      write(*,'(a,3i7,3x,3i7)') 'ERROR: Llr(ilr)%ns3, Llr(ilr)%ns2, Llr(ilr)%ns3, i1, i2, i3', Llr(ilr)%ns3, Llr(ilr)%ns2, Llr(ilr)%ns3, i1, i2, i3
                                  end if
               ...
               case('P')
                  ...
                  call partial_density(rsflag,nproc,Glr%d%n1i,Glr%d%n2i,Glr%d%n3i,&
                       npsir,nspinn,nrhotot,&
  ...
  ! Square elements in rho
  do ind=1,nrho
      rho(ind,1)=rho(ind,1)**2
  end do
  ...
  i_all=-product(shape(coeffPerOrb))*kind(coeffPerOrb)
  deallocate(coeffPerOrb,stat=i_stat)
  call memocc(i_stat,i_all,'coeffPerOrb',subname)
...
subroutine partial_density_linear(rsflag,nproc,n1i,n2i,n3i,npsir,nspinn,nrhotot,&
  ...
  !local variables
  integer :: i3s,jproc,i3off,n3d,isjmp,i1,i2,i3,i1s,i1e,j3,i3sg, iorb
  real(gp) :: hfac2, tt
  real(dp) :: psisq,p1,p2,p3,p4,r1,r2,r3,r4
        ...
        do i2=1,n2i
           ...
           if (npsir == 1) then
              do i1=i1s,i1e
                 ...
                 psisq=real(psir(i1,i2,j3,1),dp)
                 !psisq=psisq*psisq
                 ! Now sum up the charge density for all orbitals constructed
                 ! out of the localized orbitals. The squared expansion coefficients are
                 ! given by coeff.
                 tt=psisq
                 psisq=0.d0
                 do iorb=1,norbPsi
                     psisq=psisq+coeff(iorb)*tt
                 end do
                 !rho_p(i1,i2,i3s,isjmp)=rho_p(i1,i2,i3s,isjmp)+real(hfac,dp)*psisq
                 rho_p(i1,i2,i3s,isjmp)=rho_p(i1,i2,i3s,isjmp)+sqrt(real(hfac,dp))*psisq
              end do
           else !similar loop for npsir=4
              do i1=i1s,i1e
                 !conversion between the different types
