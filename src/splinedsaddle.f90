...
!  real(gp) :: fluctsum
!
!  real(gp), dimension(:,:), allocatable :: ratsp,fatsp  !REZA
!
! 
...
!        if (iproc ==0 ) write(16,*) '----------------------------------------------------------------------------'
!
!        allocate(ratsp(3,atoms%nat),fatsp(3,atoms%nat)) !REZA
!        ratsp(1:3,1:atoms%nat)=rxyz(1:3,1:atoms%nat) !REZA
!        fatsp(1:3,1:atoms%nat)=fxyz(1:3,1:atoms%nat) !REZA
!        etot=etot !REZA
!        call givemesaddle(etot,ratsp,fatsp,16,nproc,iproc,atoms,rst,inputs,ncount_bigdft) !REZA
!        close(16) !REZA
!        deallocate(ratsp,fatsp) !REZA
!
!
!        
...
module minimization_sp
    type parameterminimization_sp
        ...
        !parameters for SD and CG and SDDIIS
        real(8)::alpha=-1.d0
        real(8)::alphax=-1.d0
...
subroutine givemesaddle(epot_sp,ratsp,fatsp,ifile,nproc,iproc,atoms,rst,inputs,ncount_bigdft)
    ...
    real(gp), dimension(:,:), allocatable :: rxyz_2 !Reza
    real(gp), dimension(:,:), allocatable :: x,f,xneb,fneb,rxyz_tmp,x_t  !Reza
    integer :: np,np_neb,np_t,iat,ifile,ip,mp  !Reza
    type(parameterminimization_sp)::parmin_neb,parmin  !Reza
    real(gp) ::epot_t,epot_sp,ratsp(3,atoms%nat),fatsp(3,atoms%nat),fnoise  !Reza
    character(len=20) :: tatonam  !Reza
    integer::n,nr,istat,infocode,ios,ixyz,i,mm1,mm2,mm3
    real(8)::fnrm,fnrm1,fnrm2,calnorm,tt1,tt2,tt3,time1,time2
    ...
    enddo
    if(iproc==0) write(*,'(a,i,i)') 'DOF: n,nr ',n,nr
    !---------------------------------------------------------------------------
    ...
    allocate(fneb(n,0:np_neb+ndeb2),stat=istat);if(istat/=0) stop 'ERROR: failure allocating fneb'
    ...
    if(trim(pnow%runstat)=='restart') then
        ...
        if(np_t==np) then
            ...
            call change_np(n,nr,np_t,x_t,atoms,np,x)
        endif
        ...
        do iat=1,atoms%nat
          ...
          if(atoms%units == 'angstroemd0' .or. atoms%units == 'angstroem') then
            rxyz_tmp(1:3,iat)=rxyz_tmp(1:3,iat)/0.5291772108_gp
          endif
        enddo
        close(1336)
        ...
        do iat=1,atoms%nat
          ...
          if(atoms%units == 'angstroemd0' .or. atoms%units == 'angstroem') then
            rxyz_2(1:3,iat)=rxyz_2(1:3,iat)/0.5291772108_gp
          endif
        enddo
        close(1336)
        ...
        if(trim(pnow%doneb)=='yes') then
            ...
            call change_np(n,nr,np_neb,xneb,atoms,np,x)
        else
            call initializepoints(atoms,n,ratsp,rxyz_2,np,x)
        endif
    endif
    if(iproc==0) call writepathway(n,nr,np,x,'pathinp.xyz',atoms)
    ...
    if(trim(pnow%doneb)=='yes') then
        if(trim(pnow%runstat)=='restart') call change_np(n,nr,np,x,atoms,np_neb,xneb)
        parmin_neb%alphax=parmin%alphax
    ...
    if(trim(pnow%pickbestanchorpoints)=='yes') then
        !call pickbestanchors(n,nr,np,x,fends,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
        call pickbestanchors2(n,nr,np,x,fends,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    endif
    ...
    call splinedsaddle(n,nr,np,x,epot_sp,f,ratsp,parmin,fends,pnow, & 
        nproc,iproc,atoms,rst,inputs,ll_inputs,ncount_bigdft,fatsp)  
    if(iproc==0) call writepathway(n,nr,np,x,'pathout.xyz',atoms)
    ...
    deallocate(fneb,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating fneb'
...
!*****************************************************************************************
subroutine change_np(n,nr,np1,x1,atoms,np2,x2)
    use module_types
    !use modulesplinedsaddle, only:parametersplinedsaddle
    !use energyandforces, only:calenergyforces
    implicit none
    integer::n,nr,np1,np2,i,ip,mp,iat,ixyz
    ...
    if(np1==np2) then
        ...
        call equalarclengthparametrization(atoms,n,nr,np1,x1,s,h)
        ...
        do i=1,n
            ...
            if(move_this_coordinate(atoms%ifrztyp(iat),ixyz)) then
                y(0:np1)=x1(i,0:np1)
                call inter_cubic(np1,y,s,h,e1,e2,c)
                do ip=1,np2-1
                    tt=dt*ip
...
subroutine improvepeak(n,nr,np,x,fends,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    ...
    real(8)::tt,t1,t2,DNRM2,DDOT,fnrmtot,ed_tt,edd_tt,tarr(100),diff,dt,proj,epot,fnrm,fnoise
    ...
    call equalarclengthparametrization(atoms,n,nr,np,x,pnow%s,pnow%h)
    call factor_cubic(np,pnow%h,pnow%e1,pnow%e2)
    call fill_ex_exd(0,n,nr,np,x,fends,npv,pnow,pold,xt,ft,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    !call guessinitialtmax_hermite(iproc,0,npv,pnow)
    call guessinitialtmax_cubic(0,npv,pnow)
    ...
    do i=1,n
        ...
        if(move_this_coordinate(atoms%ifrztyp(iat),ixyz)) then
            ...
            call inter_cubic(np,pnow%y,pnow%s,pnow%h,pnow%e1,pnow%e2,pnow%c)
    ...
    do iter=1,10
        ...
        call atomic_dot(atoms,ft,xt,proj)
        if(iproc==0) then
...
!*****************************************************************************************
subroutine pickbestanchors2(n,nr,np,x,fends,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use modulesplinedsaddle, only:parametersplinedsaddle
    !use energyandforces, only:calenergyforces
    implicit none
    integer::n,nr,np,i,ip,istat,npv,nproc,iproc,mp,ncount_bigdft,ixyz,iat,icycle,ncycle
    real(8)::x(n,0:np),fends(n,2) !,f(n,0:np),calnorm
    type(atoms_data), intent(inout) :: atoms
    type(input_variables), intent(inout) :: ll_inputs
    type(restart_objects), intent(inout) :: rst
    real(8)::tt,t1,t2,DNRM2,DDOT,fnrmtot,ed_tt,edd_tt,tarr(100),diff,dt
    real(8), allocatable::xt(:),ft(:)
    type(parametersplinedsaddle)::pnow,pold
    real(8)::ttmin,ttmax,elin,emax,emin,e1,e2,t,exo(0:100),exn(0:100),area,areatot
    real(8)::s_t(0:100)
    integer, parameter::ndeb1=0,ndeb2=0
    logical::move_this_coordinate
    if(mod(np+pnow%ns2,2)==0) then
...
!*****************************************************************************************
subroutine pickbestanchors(n,nr,np,x,fends,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use modulesplinedsaddle, only:parametersplinedsaddle
    !use energyandforces, only:calenergyforces
    implicit none
    integer::n,nr,np,i,ip,istat,npv,nproc,iproc,mp,ncount_bigdft,ixyz,iat
    real(8)::x(n,0:np),fends(n,2) !,f(n,0:np),calnorm
    type(atoms_data), intent(inout) :: atoms
    type(input_variables), intent(inout) :: ll_inputs
    type(restart_objects), intent(inout) :: rst
    ...
    call factor_cubic(np,pnow%h,pnow%e1,pnow%e2)
    call fill_ex_exd(0,n,nr,np,x,fends,npv,pnow,pold,xt,ft,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    !call fill_ex_exd(istep,n,nr,np,x,fends,npv,pnow,pold,xt,ft,nproc,iproc,atoms,rst,inputs,ncount_bigdft)
    call guessinitialtmax_hermite(iproc,0,npv,pnow)
...
!*****************************************************************************************
subroutine neb(n,nr,np,x,etmax,f,xtmax,parmin,fends,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use minimization_sp, only:parameterminimization_sp
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::n,nr,np,mp,i,ip,icall,istat,m,it,it2,nwork,infocode,iat,ixyz,ir
    real(8)::x(n,0:np),f(n,0:np),fends(n,2),etmax,xtmax(n),calnorm
    real(8)::fnrm,fspmax,fspnrm,calmaxforcecomponent,t1,fnrmtot
    real(8), allocatable::diag(:),work(:)
    ...
    if(trim(parmin%approach)=='SD') then
        ...
        do it=1,parmin%maxforcecall
            call nebforce(n,nr,np,x,f,fnrmtot,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
            call calmaxforcecomponentanchors(atoms,np,f(1,1),fnrm,fspmax)
            fa(1:nr,1:np-1)=f(1:nr,1:np-1)
            call checkconvergence(iproc,parmin,fnrm,fspmax)
    ...
    if(trim(parmin%approach)=='SDCG') then
        ...
        do it=1,parmin%maxforcecall
            ...
            fa(1:nr,1:np-1)=f(1:nr,1:np-1)
            call checkconvergence(iproc,parmin,fnrm,fspmax)
            if(.not. parmin%sdsaturated) then
                !call sdminimum(atoms,iproc,n,np,nr*(np-1),xa,fa,fnrmtot,parmin,nwork,work)
            call sdminimum(iproc,nr*(np-1),nr*(np-1),xa,fa,fnrmtot,parmin,nwork,work)
                if(iproc==0 .and. parmin%converged) then
                    write(pnow%ifile,*) 'converged before starting CG'
                    write(*,*) 'converged before starting CG'
                endif
                if(parmin%itsd>parmin%nitsd .and. .not. parmin%sdsaturated) then
                if(iproc==0) then
                    write(pnow%ifile,'(a)') 'SD did not saturate, so cgminimum can not continue.'
                    write(*,'(a)') 'SD did not saturate, so cgminimum can not continue.'
                endif
                endif 
            elseif(.not. parmin%converged) then
                !call cgminimum(atoms,iproc,n,np,nr*(np-1),xa,fa,etmax,parmin,nwork,work)
                call cgminimum(iproc,nr*(np-1),nr*(np-1),xa,fa,fnrmtot,parmin,nwork,work)
            endif
            do ip=1,np-1
                call atomic_copymoving_backward(atoms,nr,xa(1,ip),n,x(1,ip))
            enddo
    ...
    if(trim(parmin%approach)=='FIRE') then
        ...
        do it=1,parmin%maxforcecall
            ...
            !call calvmaxanchorforces(icall,n,nr,np,x,xold,fends,etmax,f,xtmax,pnow,pold,fatsp, &
            ...
            !endif
            !if(it<=10) then
            !call perpendicularforce(n,nr,np,x,f,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
            !endif
            !!if(iproc==0 .and. it==1) then
            !if(iproc==0) then
            !do ip=1,np-1
...
!*****************************************************************************************
subroutine nebforce(n,nr,np,x,f,fnrmtot,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use modulesplinedsaddle, only:parametersplinedsaddle
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::n,nr,np,i,ip,istat,infocode
    real(8)::x(n,0:np),f(n,0:np),calnorm,fnoise
    real(8)::tt,t1,t2,springcons,dnrm2,ddot,fnrmtot,time1,time2
    ...
    enddo
    deallocate(x_bigdft,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating x_bigdft.'
    call caltangentupwind(n,nr,np,x,pnow%ex,tang)
    springcons=5.d-2
    fnrmtot=0.d0
    do ip=1,np-1
...
subroutine splinedsaddle(n,nr,np,x,etmax,f,xtmax,parmin,fends,pnow,nproc, &
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::n,nr,np,mp,i,ip,icall,istat,m,it,it2,nwork,infocode
    real(8)::x(n,0:np),f(n,0:np),fends(n,2),etmax,xtmax(n),calnorm,fatsp(n)
    real(8)::fnrm,fnrm1,fnrm2,fspmax,fspnrm,calmaxforcecomponent,barrier1,barrier2
    real(8), allocatable::diag(:),work(:)
    ...
    if(trim(parmin%approach)=='SDCG') then
        ...
        do it=1,parmin%maxforcecall
            ...
            elseif(.not. parmin%converged) then
                !call cgminimum(atoms,iproc,n,np,nr*(np-1),xa,fa,etmax,parmin,nwork,work)
                call cgminimum(iproc,nr*(np-1),nr*(np-1),xa,fa,etmax,parmin,nwork,work)
            endif
            do ip=1,np-1
                call atomic_copymoving_backward(atoms,nr,xa(1,ip),n,x(1,ip))
            enddo
    ...
    if(trim(parmin%approach)=='SDDIIS') then
        ...
        icall=0
        do it=1,parmin%maxforcecall
            call calvmaxanchorforces(icall,n,nr,np,x,xold,fends,etmax,f,xtmax,pnow,pold, &
                fatsp,parmin,nproc,iproc,atoms,rst,inputs,ll_inputs,ncount_bigdft)
            call calmaxforcecomponentsub(atoms,fatsp,fspnrm,fspmax)
            call reportcalvmaxanchorforces(iproc,icall,n,np,x,etmax,fspnrm,fspmax,pnow,atoms,ncount_bigdft)
            call checkconvergence(iproc,parmin,fnrm,fspmax)
            do ip=1,np-1
    ...
    if(trim(parmin%approach)=='LBFGS') then
        ...
        parmin%DIAGCO=.false.
        parmin%iflag=0
        allocate(diag(nr*(np-1)+ndeb1),stat=istat)
        if(istat/=0) stop 'ERROR: failure allocating diag.'
        call dmemocc(nr*(np-1),nr*(np-1)+ndeb1,diag,'diag')
        allocate(xold(n,0:np+ndeb2),stat=istat)
        if(istat/=0) stop 'ERROR: failure allocating xold.'
        call dmemocc(n*(np+1),n*(np+1+ndeb2),xold,'xold')
        allocate(work(nr*(np-1)*(2*m+1)+2*m+ndeb1),stat=istat)
        if(istat/=0) stop 'ERROR: failure allocating work.'
        call dmemocc(nr*(np-1)*(2*m+1)+2*m,nr*(np-1)*(2*m+1)+2*m+ndeb1,work,'work')
        icall=0
        do it=1,parmin%maxforcecall
            call calvmaxanchorforces(icall,n,nr,np,x,xold,fends,etmax,f,xtmax,pnow,pold,fatsp, &
                parmin,nproc,iproc,atoms,rst,inputs,ll_inputs,ncount_bigdft)
            ...
            do ip=1,np-1
                call atomic_copymoving_forward(atoms,n,f(1,ip),nr,fa(1,ip))
            enddo
            call lbfgs_sp(iproc,nr*(np-1),m,xa,etmax,fa,diag,work,parmin) !,nr) !,fatsp)
            do ip=1,np-1
                call atomic_copymoving_backward(atoms,nr,xa(1,ip),n,x(1,ip))
            enddo
            if(parmin%converged) exit
...
subroutine bfgs_splsad(iproc,nr,x,epot,f,nwork,work,parmin)
    ...
    write(*,'(a10,i4,es23.15,es11.3,2es12.5,1es12.4)') &
        'BFGSMIN   ',parmin%iter,epot,de,fnrm,fmax,alpha
    endif
    ...
    if(parmin%iter==0 .or. reset) then
        ...
        work(1:nr*nr)=0.d0
        do i=1,nr
            work(i+(i-1)*nr)=1.d0/parmin%alphax
        enddo
        work(iw3:iw3-1+nr)=parmin%alphax*f(1:nr)
    else
...
!*****************************************************************************************
subroutine perpendicularforce(n,nr,np,x,f,pnow,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use modulesplinedsaddle, only:parametersplinedsaddle
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::n,nr,np,i,ip,istat,infocode,mp
    real(8)::x(n,0:np),f(n,0:np),calnorm,fnoise,epotarr(0:100)
    type(parametersplinedsaddle)::pnow
    real(8)::tt,t1,t2,DNRM2,DDOT,fnrm,fnrmmax,time1,time2
    ...
    call caltangentupwind(n,nr,np,x,epotarr,tang)
    !do ip=1,np-1
    do ip=mp,mp
...
!*****************************************************************************************
subroutine calvmaxanchorforces(istep,n,nr,np,x,xold,fends,etmax,f,xtmax,pnow,pold,ftmax,parmin, &
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::n,nr,np,mp,i,ip,j,infocode
    real(8)::x(n,0:np),xold(n,0:np),fends(n,2),f(n,0:np),xtmax(n),ftmax(n)
    integer::istat,istep
    type(parametersplinedsaddle)::pnow,pold
    type(parameterminimization_sp)::parmin
    real(8)::fnrm,fnrm1,fnrm2,etmax,epot_tmp,tt,fnoise,time1,time2
    ...
    call caltmax2(istep,n,nr,np,x,xold,fends,etmax,xtmax,ftmax,pnow,pold,nproc,iproc,&
...
subroutine checkpathway(iproc,istep,n,np,x,xold,pnow)
    use modulesplinedsaddle, only:parametersplinedsaddle
    implicit none
    integer::iproc,istep,n,np,npv,ip,i
...
!*****************************************************************************************
!subroutine splinedsaddle(n,nr,np,x,etmax,f,xtmax,parmin)
!    use modulesplinedsaddle, only:parametersplinedsaddle
!    implicit none
!    integer::n,nr,np,mp,i,ip
!    real(8)::x(n,0:np),f(n,0:np),xtmax(n),calnorm
!    integer::istat,istep
!    !integer, save::istep=0
!    type(parametersplinedsaddle)::pnow,pold
!    real(8)::fnrm,fnrm1,fnrm2,etmax
!    real(8), allocatable::ftmax(:)
!    real(8), allocatable::dd(:,:)
!    real(8), allocatable::fends(:,:)
!    real(8), allocatable::xa(:,:),fa(:,:)
!    !----------------------------------------
!    integer::iflag,iprint(2),nsave
!    real(8)::STPMAX
!    logical::diagco
!    external lb2
!    COMMON /LB3/STPMAX
...
!    iprint(2)=0
!    !----------------------------------------
!    allocate(xa(nr,np-1),fa(nr,np-1),stat=istat)
!    if(istat/=0) stop 'ERROR: failure allocating xa or fa.'
!    allocate(dd(n,np-1),stat=istat);if(istat/=0) stop 'ERROR: failure allocating dd.'
!    allocate(ftmax(n),stat=istat);if(istat/=0) stop 'ERROR: failure allocating ftmax.'
!    allocate(fends(n,2),stat=istat);if(istat/=0) stop 'ERROR: failure allocating ftmax.'
!    !----------------------------------------
!    allocate(diag(n*(np-1)),stat=istat);if(istat/=0) stop 'ERROR: failure allocating diag.'
!    allocate(work(n*(np-1)*(2*nsave+1)+2*nsave),stat=istat);if(istat/=0) stop 'ERROR: failure allocating work.'
!    !----------------------------------------
!    call calenergyforces(n,x(1,0),pnow%exends(1),fends(1,1))
!    fnrm=calnorm(nr,fends(1,1))
!    write(*,*) 'end one: fnrm',fnrm
!    call calenergyforces(n,x(1,np),pnow%exends(2),fends(1,2))
!    fnrm=calnorm(nr,fends(1,2))
!    write(*,*) 'end one: fnrm',fnrm
!    xa(1:nr,1:np-1)=x(1:nr,1:np-1)
!    do istep=0,1000
!        call equalarclengthparametrization(atoms,n,nr,np,x,pnow%s,pnow%h)
...
!        !--------------------------------------------------------
!        !call caltmax(n,np,x,etmax,xtmax,ftmax,pnow,pold)
!        call caltmax2(istep,n,nr,np,x,fends,etmax,xtmax,ftmax,pnow,pold)
!        !--------------------------------------------------------
!        call calindex(np,pnow%s,pnow%tmax,mp)
!        write(*,'(a,i5,2e24.15)') 'SP ',istep,xtmax(1),xtmax(2)-1.d0
!        write(31,*) xtmax(1),xtmax(2)
!        !-----------------------------------------------------------------------
!        call prepdd(n,nr,np,x,pnow%e1,pnow%e2,pnow%h,pnow%s,mp,pnow%tmax,dd)
!        do ip=1,np-1;do i=1,n;f(i,ip)=ftmax(i)*dd(i,ip);enddo;enddo
!        !-----------------------------------------------------------------------
!        fnrm1=calnorm(nr,ftmax)
!        fa(1:nr,1:np-1)=-f(1:nr,1:np-1)
!        fnrm2=calnorm(nr*(np-1),fa(1,1))
!        write(*,'(a,i6,3e24.15)') 'istep,vmax,fnrm1,fnrm2 ',istep,etmax,fnrm1,fnrm2
!        if(fnrm2<1.d-4) exit
!        !-----------------------------------------------------------------------
!        if(istep>-1) then
!        !if(fnrm2<1.d-2) then
!        !f(1:n,1:np-1)=-f(1:n,1:np-1)
!        call lbfgs_sp(nr*(np-1),nsave,xa,etmax,fa,diagco,diag,iprint,1.d-5,1.d-16,work,iflag)
!        do ip=1,np-1
!            do i=1,nr
!                x(i,ip)=xa(i,ip)
!            enddo
...
!        if(iflag<=0) exit
!        else
!        x(1:n,1:np-1)=x(1:n,1:np-1)+min(1.d-1/fnrm2,5.d-3)*f(1:n,1:np-1)
!        endif
!        pold=pnow
!    enddo
!    !write(*,*) 'barrier heights',etmax-pnow%exends(1),etmax-pnow%exends(2)
!    deallocate(xa,fa,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating xa or fa.'
!    deallocate(diag,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating diag.'
!    deallocate(work,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating work.'
!    deallocate(ftmax,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating ftmax.'
!    deallocate(dd,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating dd.'
!    deallocate(fends,stat=istat);if(istat/=0) stop 'ERROR: failure deallocating dd.'
!end subroutine splinedsaddle
!*****************************************************************************************
...
!*****************************************************************************************
subroutine caltmax2(istep,n,nr,np,x,xold,fends,epot,xt,ft,pnow,pold,nproc,iproc,atoms, &
    ...
    if(iproc==0) then
        ...
        call writepathway(n,nr,np,x,filename,atoms)
    endif
    !--------------------------------------------------------
...
1358 continue
    ...
    if(istep<1 .or. pnow%do_fill_ex_exd) then
        call fill_ex_exd(istep,n,nr,np,x,fends,npv,pnow,pold,xt,ft,nproc,iproc,atoms, &
            rst,ll_inputs,ncount_bigdft)
        !call guessinitialtmax_quintic(istep,npv,pnow,iproc)
        !call guessinitialtmax_hermite(iproc,istep,npv,pnow)
        call guessinitialtmax_cubic(istep,npv,pnow)
        xold(1:n,0:np)=x(1:n,0:np)
    else
        npv=pold%npv
    ...
    call func(pnow%tmax,epot,vd,n,nr,np,x,pnow,mp,xt,ft,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    ...
    if(pnow%typintpol=='cubic') then
        ...
        call calv_quadratic(iproc,istep,npv,pnow,mpv,vc,vdc,vddc)

        !call factor_cubic(npv,pnow%hv,pnow%e1v,pnow%e2v)
        !call inter_cubic(npv,pnow%ex,pnow%sv,pnow%hv,pnow%e1v,pnow%e2v,pnow%cv)
        ...
        !call factor_inter_quintic(npv,pnow%sv,pnow%hv,pnow%ex,pnow%exd,pnow%a,pnow%b)
...
subroutine write_v_of_t(iproc,istep,npv,pnow,icall,fn)
    ...
    do ipv=0,500
        ...
        !call ffdfdd_hermite(iproc,istep,npv,pnow%sv,pnow%hv,pnow%ex,pnow%exd,t,mpv,vc,vdc,vddc)
        if(iproc==0) write(2010,'(i5,4es19.10,i4)') ipv,t,vc,vdc,vddc,npv
    enddo
    if(iproc==0) close(2010)
end subroutine write_v_of_t
!*****************************************************************************************
subroutine ffdfdd_hermite(iproc,istep,n,s,h,e,ed,t,m,v,vd,vdd)
    implicit none
    integer::iproc,istep,n,m
...
subroutine fdd_quadratic(n,s,h,e,t,m,vd,vdd)
    ...
    real(8)::delta(100),avg,w1,w2
    real(8)::f0,f1,f2,h1,h2,vdt,vddt,vdddt,a0,a1,a2,t1
    h1=h(m)
    h2=h(m+1)
    t1=s(m)
    f0=e(m-1)
...
!*****************************************************************************************
subroutine calv_quadratic(iproc,istep,npv,pnow,mpv,vc,vdc,vddc)
    use modulesplinedsaddle, only:parametersplinedsaddle
    implicit none
...
subroutine calv_hermite(iproc,istep,npv,pnow,mpv,vc,vdc,vddc)
    ...
    real(8)::vc,vdc,vddc
    character(100)::text
    call ffdfdd_hermite(iproc,istep,npv,pnow%sv,pnow%hv,pnow%ex,pnow%exd,pnow%tmax,mpv,vc,vdc,vddc)
...
!*****************************************************************************************
subroutine guessinitialtmax_hermite(iproc,istep,npv,pnow)
    use modulesplinedsaddle, only:parametersplinedsaddle
    implicit none
    integer::iproc,istep,npv,mpv,ipvt,istat,iroot,nroot,npvt
    type(parametersplinedsaddle)::pnow
    real(8)::p1,p2,p3,t1,t2,hi,discriminant,t,dt,v,vcmax,vddq,roots(50)
    ...
    !ffdfdd_hermite(iproc,istep,n,s,h,e,ed,t,m,v,vd,vdd)
    !call factor_cubic(npvt,hvt,e1vt,e2vt)
    !call inter_cubic(npvt,ext,svt,hvt,e1vt,e2vt,cvt)
    nroot=0
    do ipvt=1,npvt
        hi=hvt(ipvt)
    ...
    do iroot=1,nroot
        ...
        call ffdfdd_hermite(iproc,istep,npvt,svt,hvt,ext,exdt,roots(iroot),ipvt,v,t1,t2)
        if(v>vcmax) then
            vcmax=v
...
!*****************************************************************************************
subroutine fill_ex_exd(istep,n,nr,np,x,fends,npv,pnow,pold,xt,ft,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use modulesplinedsaddle, only:parametersplinedsaddle
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::istep,n,nr,np,ip,mp,istat,i,npv,mpt,infocode
    real(8)::x(n,0:np),fends(n,2),xt(n),ft(n),dt,tnearest
    type(parametersplinedsaddle)::pnow,pold
    real(8)::mydot,t1,tt,tt1,tt2,fnoise,exmax,exmin,dtt,time1,time2
    real(8), allocatable::tang(:),x_bigdft(:)
    logical::move_this_coordinate
    integer::iat,ixyz
    integer, parameter::ndeb1=0,ndeb2=0
...
subroutine estimate_sv(iproc,istep,np,npv,pnow,pold)
    use modulesplinedsaddle, only:parametersplinedsaddle
    implicit none
    integer::iproc,istep,np,npv,ip,mpl,mpr,jp,mp,icycle,ncycle
    type(parametersplinedsaddle)::pnow,pold
    real(8)::dt,tt,ttl,ttr,t1,t2,tt1,tt2,e_end_min
    real(8)::vi1,vi2,h,hinv,a0,a1,a2,a3,el,er,edl,edr
    real(8)::ttmin,ttmax,elin,emax,emin,e1,e2,t,exo(0:100),exn(0:100),area,areatot
    pnow%sv(0)=0.d0
    pnow%sv(npv)=pnow%s(np)
    if(istep>0) then
...
!*****************************************************************************************
subroutine guessinitialtmax_cubic(istep,npv,pnow)
    use modulesplinedsaddle, only:parametersplinedsaddle
    implicit none
    integer::istep,npv,mpv,ipvt,istat,iroot,nroot,npvt
    type(parametersplinedsaddle)::pnow
...
!*****************************************************************************************
subroutine guessinitialtmax_quintic(istep,npv,pnow,iproc)
    use modulesplinedsaddle, only:parametersplinedsaddle
    implicit none
    integer::istep,npv,mpv,ipvt,istat,iroot,nroot,npvt,iproc
    type(parametersplinedsaddle)::pnow
    real(8)::p1,p2,p3,t1,t2,hi,discriminant,t,dt,vc,vcmax,vddq,roots(50)
    real(8), allocatable::svt(:),hvt(:),ext(:),e1vt(:),e2vt(:),cvt(:)
    integer, parameter::ndeb1=0,ndeb2=0
...
!*****************************************************************************************
subroutine factor_inter_quintic(n,s,h,y,d,a,b)
    implicit none
    integer::n,i,j,k,info
    integer, parameter::kl=2,ku=2
    real(8)::s(0:n),h(n),y(0:n),d(0:n),a(n),b(n),t1,t2,t3,t4
...
subroutine prepdd(atoms,n,np,x,e1,e2,h,s,mp,tmax,dd)
    ...
    real(8)::x(n,0:np),e1(np-1),e2(np-2),h(np),s(0:np),dd(n,n,np-1),tmax,t1,t2
    ...
    do j=1,n
        ...
        yj(0:np)=x(j,0:np)
        call inter_cubic(np,yj,s,h,e1,e2,c)
        !t1=0.d0
        !do ip=1,np-1
...
subroutine prepcd3cd4(np,h,mp,ainv,i,j,yi,yj,cd1,cd2)
    implicit none
    integer::np,mp,istat,i,j,info,ip,jp
...
!*****************************************************************************************
subroutine func(tt,epot,ett,n,nr,np,x,pnow,mp,xt,ft,nproc,iproc,atoms,rst,ll_inputs,ncount_bigdft)
    use module_base
    use module_interfaces, except_this_one => geopt
    use module_types
    use modulesplinedsaddle, only:parametersplinedsaddle
    ...
    type(restart_objects), intent(inout) :: rst
    integer, intent(inout) :: ncount_bigdft
    integer::n,nr,np,mp,i,istat,infocode
    type(parametersplinedsaddle)::pnow
    real(8)::tt,ett,x(n,0:np),epot,mydot,xt(n),ft(n),t1,fnoise,time1,time2
    real(8), allocatable::tang(:),x_bigdft(:)
    logical::move_this_coordinate
    integer::ixyz,iat
    integer, parameter::ndeb1=0,ndeb2=0
...
!*****************************************************************************************
subroutine equalarclengthparametrization(atoms,n,nr,np,x,s,h)
    use module_types
    implicit none
    type(atoms_data), intent(inout) :: atoms
    integer::n,nr,np
    real(8)::x(n,0:np),s(0:np),h(np),tt,snpinv
...
!*****************************************************************************************
subroutine inter_cubic(np,y,s,h,e1,e2,c)
    implicit none
    integer::np
    integer::ip,jp,kp,istat,j,is,info
    real(8)::y(0:np),s(0:np),h(np),e1(np-1),e2(np-2),c(0:np)
...
!*****************************************************************************************
subroutine caltangentupwind(n,nr,np,x,ex,tang)
    implicit none
    integer::n,nr,np,ip
...
!*******************************************************************************
subroutine checkconvergence(iproc,parmin,fnrm,fspmax)
    use minimization_sp, only:parameterminimization_sp
    implicit none
    integer::iproc
    real(8)::fnrm,fspmax
...
!*******************************************************************************
subroutine initsdminimum(iproc,n,nr,x,parmin,nwork,work)
    use minimization_sp, only:parameterminimization_sp
    implicit none
...
subroutine fire(iproc,nr,x,epot,f,work,parmin)
    ...
    tt=min(alpha*vnrm/fnrm,1.d-1*parmin%alphax)
...
subroutine sdminimum(iproc,n,nr,x,f,epot,parmin,nwork,work)
    ...
    logical::xmoved
    if(parmin%iflag==0) call initsdminimum(iproc,n,nr,x,parmin,nwork,work)
    fnrm=calnorm(nr,f);fmax=calmaxforcecomponent(nr,f)
...
!*******************************************************************************
subroutine cgminimum(iproc,n,nr,x,f,epot,parmin,nwork,work)
    use minimization_sp, only:parameterminimization_sp
    implicit none
    type(parameterminimization_sp)::parmin
    integer::iproc,n,nr,nwork
    real(8)::x(n),f(n),epot,work(nwork),calnorm
    real(8)::fnrm,tt,y0,y1,mydot,rlambda,t1,t2,t3,calmaxforcecomponent,fmax
    character(34), parameter::frt1= '(a10,i4,e23.15,e11.3,2e12.5,e12.4)'
    !logical, save::
    if(parmin%iflag==0) then
        parmin%avgalpha=0.d0;parmin%avgnum=0.d0;parmin%ifail=0;parmin%iflag=1;parmin%dolinesearch=.true.
        parmin%alpha0=4.d0*parmin%alphax;parmin%epotitm1=epot;parmin%itcg=0;rlambda=0.d0;parmin%alpha=0.d0
        work(1:nwork)=0.d0
        work(nr+1:nr+nr)=f(1:nr)
    endif
    if(parmin%dolinesearch) then
        if((epot-parmin%epotitm1)>parmin%anoise) then
            if(iproc==0) then
                write(parmin%ifile,'(a35,i5,2e25.15)') &
                    'back to SD:itcg,epot,epot-epotitm1',parmin%itcg,epot,epot-parmin%epotitm1
                write(*           ,'(a35,i5,2e25.15)') &
                    'back to SD:itcg,epot,epot-epotitm1',parmin%itcg,epot,epot-parmin%epotitm1
            endif
            x(1:nr)=work(1:nr)
            parmin%sdsaturated=.false.
        ...
        t1=mydot(nr,f,f);t2=mydot(nr,work(nr+1),f)
        t3=mydot(nr,work(nr+1),work(nr+1));rlambda=(t1-t2)/t3 
        work(2*nr+1:2*nr+nr)=f(1:nr)+rlambda*work(2*nr+1:2*nr+nr)
        fnrm=sqrt(t1)
        fmax=calmaxforcecomponent(nr,f)
        if(iproc==0) then
            write(parmin%ifile,frt1) 'CGMIN     ', &
                parmin%itcg,epot,epot-parmin%epotitm1,fnrm,fmax,parmin%alpha/parmin%alphax
            write(*           ,frt1) 'CGMIN     ', &
                parmin%itcg,epot,epot-parmin%epotitm1,fnrm,fmax,parmin%alpha/parmin%alphax
        endif
        parmin%epotitm1=epot
        if(parmin%itcg==0) then
            parmin%alpha0=4.d0*parmin%alphax
        else
            parmin%alpha0=max(min(2.d0*parmin%alpha,4.d0*parmin%alphax),-parmin%alphax)
        endif
        !if(fmax<parmin%fmaxtol) then
        !    parmin%converged=.true.
        !    parmin%iflag=0
        !endif
        if(parmin%itcg==500) then
            !write(*,*) 'NO conv in CG after 500 its: switching back to SD',itcg,fnrm,epot
            x(1:nr)=work(1:nr)
            f(1:nr)=work(nr+1:nr+nr)
            parmin%ifail=parmin%ifail+1
            if(parmin%ifail==parmin%nfail) stop 'too many failures of cgminimum'
            parmin%sdsaturated=.false.
            return
        endif
        work(1:nr)=x(1:nr)
        x(1:nr)=x(1:nr)+parmin%alpha0*work(2*nr+1:2*nr+nr)
        work(nr+1:nr+nr)=f(1:nr)
        parmin%dolinesearch=.false.
        parmin%itcg=parmin%itcg+1
    else
        y0=mydot(nr,work(nr+1),work(2*nr+1));y1=mydot(nr,f,work(2*nr+1));tt=y0/(y0-y1)
        !write(*,'(a,2(1x,e10.3),2x,e12.5)')  'y0,y1,y0/(y0-y1)',y0,y1,tt
        parmin%alpha=parmin%alpha0*max(min(tt,6.d0),-5.d0)
        !parmin%alpha=parmin%alpha0*max(min(tt,2.d0),-0.25d0)
        !parmin%alpha=parmin%alpha0*max(min(tt,10.d0),-1.d0)
        x(1:nr)=work(1:nr)+parmin%alpha*work(2*nr+1:2*nr+nr)
        parmin%avgalpha=parmin%avgalpha+parmin%alpha/parmin%alphax
        parmin%avgnum=parmin%avgnum+1.d0
        parmin%dolinesearch=.true.
    endif
    !write(*,*) 'average CG stepsize in terms of alphax',parmin%avgalpha/parmin%avgnum
...
!*******************************************************************************
subroutine writepathway(n,nr,np,x,filename,atoms)
    use module_types
    implicit none
    integer::n,nr,np,jp,kp,mpt,nskip,iposout,iat
    real(8)::x(n,0:np),hi,epotmax,ed_tt,edd_tt,dtt,tt,x1,x2,xyz(3)
    integer::istat,ip,i,info,k1,k2,j,lwork
    type(atoms_data), intent(in) :: atoms
    ...
    call dmemocc(n,n+ndeb1,xt,'xt')
    !-------------------------------------------------------------------------------------------
    call equalarclengthparametrization(atoms,n,nr,np,x,s,h)
    ...
    do jp=0,100
        ...
        do i=1,n
            ...
            if(move_this_coordinate(atoms%ifrztyp(iat),ixyz)) then
                y(0:np)=x(i,0:np)
                call inter_cubic(np,y,s,h,e1,e2,c)
            ...
            if(mod(i,3)==0) then
                ...
                if(atoms%units=='angstroemd0' .or. atoms%units=='angstroem') then
                    xyz(1:3)=xt(i-2:i)*0.5291772108_gp
                else
                    xyz(1:3)=xt(i-2:i)
                endif
...
subroutine writeanchorpoints(n,np,x,filename,atoms)
    use module_types
    implicit none
    integer::n,np,kp,mpt,nskip,iposout,iat
    real(8)::x(n,0:np),hi,epotmax,ed_tt,edd_tt,dtt,tt,x1,x2,xyz(3)
    integer::istat,ip,i,info,k1,k2,j,lwork
    type(atoms_data), intent(in) :: atoms
    character(len=10) :: name
    ...
    do ip=0,np
        ...
        do i=1,n
            if(mod(i,3)==0) then
                ...
                if(atoms%units=='angstroemd0' .or. atoms%units=='angstroem') then
                    xyz(1:3)=x(i-2:i,ip)*0.5291772108_gp
                else
                    xyz(1:3)=x(i-2:i,ip)
                endif
    ...
    do ip=0,np
        ...
        do i=1,n
            if(mod(i,3)==0) then
                ...
                if(atoms%units=='angstroemd0' .or. atoms%units=='angstroem') then
                    x(i-2:i,ip)=xyz(1:3)/0.5291772108_gp
                else
                    x(i-2:i,ip)=xyz(1:3)
                endif
            endif
...
!*****************************************************************************************
subroutine lbfgs_sp(iproc,n,m,x,f,g,diag,w,parmin) !,n_sp) !,f_sp)
    use minimization_sp, only:parameterminimization_sp
    implicit none
    type(parameterminimization_sp)::parmin
    integer::n,m,iproc !,n_sp
    real(8)::x(n),g(n),diag(n),w(n*(2*m+1)+2*m),f !,f_sp(n_sp)
    real(8)::one,zero,gnorm,ddot,stp1,xnorm,beta,yr,sq,yy !,calmaxforcecomponent !,fmax
    integer::npt,cp,i,inmc,iycn,iscn
    real(8), save::ys,a_t
    integer, save::bound,nfun,nfev,point,iypt,ispt  !,info
    logical, save::finish !,new
    data one,zero/1.0d+0,0.0d+0/
    if(parmin%iflag==0) then
        call init_lbfgs_sp(iproc,n,m,x,f,g,diag,w,parmin,nfun,point,finish,stp1,ispt,iypt,a_t)
    endif
    parmin%new=.false.
    if(parmin%iflag==0) parmin%new=.true.
    !---------------------------------------------------
    !fmax=calmaxforcecomponent(n_sp,f_sp)
    if(parmin%converged) then
    !if(fmax<parmin%fmaxtol) then
        if(iproc==0) write(*,*) 'REZA: in if statement',parmin%iter
        finish=.true.
        gnorm=sqrt(ddot(n,g,1,g,1))
        if(parmin%iprint>=0) call lb1_sp(iproc,nfun,gnorm,n,m,x,f,g,a_t,finish,parmin)
        parmin%iflag=0
        return
    endif
    !---------------------------------------------------
    ...
    do 
        ...
        if((parmin%new .or. parmin%iflag==2) .and. parmin%iter/=1) then
            !write(*,*) 'ALI',iflag,iter,parmin%new
            if(parmin%diagco) then
                do i=1,n
                    if(diag(i)<=zero) then
                        parmin%iflag=-2
                        if(iproc==0) then
                                write(parmin%ifile,'(a,i5,2a)') &
                            'iflag=-2, the',i,'-th diagonal element ', &
        ...
        if(parmin%iflag/=1 .or. parmin%new) then
            nfev=0
            a_t=one
        ...
        !write(*,*) 'a_t',a_t
        call mcsrch_sp(n,x,f,g,w(ispt+point*n+1),a_t,parmin%info,nfev,diag,parmin)
        if (parmin%info==-1) then
            parmin%iflag=1
            return
        endif
        if(parmin%info/=1) then
            parmin%iflag=-1
            if(iproc==0) then
                    write(parmin%ifile,'(2a)') 'iflag=-1, line search failed. ', & 
                ' see documentation of routine mcsrch.'
                    write(*,'(2a)') 'iflag=-1, line search failed. ', & 
                ' see documentation of routine mcsrch.'
            endif
            if(iproc==0) write(*,'(a,i2)') 'error return of line search: info= ',parmin%info
            if(iproc==0) write(*,'(2a)') 'possible causes: ', &
        ...
        !fmax=calmaxforcecomponent(n_sp,f_sp)
        !if(fmax<parmin%fmaxtol) finish=.true.
        ...
        parmin%new=.true.
    enddo
end subroutine lbfgs_sp
!*****************************************************************************************
subroutine init_lbfgs_sp(iproc,n,m,x,f,g,diag,w,parmin,nfun,point,finish,stp1,ispt,iypt,a_t)
    use minimization_sp, only:parameterminimization_sp
    implicit none
    type(parameterminimization_sp)::parmin
    integer::n,m,i,iproc
    ...
    if(n<1 .or. m<1) then
        parmin%iflag= -3
        if(iproc==0) write(*,'(a)') &
            'iflag= -3, improper input parameters(n or m are less than 1)'
        return
    endif
    if(parmin%gtol<1.d-4) then
        if(parmin%lp>0) then
        if(iproc==0) write(*,'(a)') &
            'gtol is less than 1.d-4, it has been reset to 9.d-1'
        endif
        parmin%gtol=9.d-01
    endif
    nfun=1
    ...
    if(parmin%diagco) then
        do i=1,n
            if(diag(i)<=zero) then
                parmin%iflag=-2
                if(iproc==0) write(*,'(a,i5,2a)') &
                    'iflag=-2, the',i, '-th diagonal element of ', &
    ...
    stp1=5.d-2/gnorm
    if(parmin%iprint>=0) call lb1_sp(iproc,nfun,gnorm,n,m,x,f,g,a_t,finish,parmin)
end subroutine init_lbfgs_sp
!*****************************************************************************************
subroutine lb1_sp(iproc,nfun,gnorm,n,m,x,f,g,a_t,finish,parmin)
    use minimization_sp, only:parameterminimization_sp
    type(parameterminimization_sp)::parmin
    !this routine prints monitoring information. the frequency and
    !amount of output are controlled by iprint.
    integer::nfun,n,m,iproc
    real(8)::x(n),g(n),f,gnorm,a_t,fmax,calmaxforcecomponent,de1
    logical::finish
    character(34), parameter::frt1='(a10,i4,e23.15,e11.3,2e12.5,e12.4)'
    character(70)::text
    if(iproc==0) write(*,*) 'ALI: in if statement',parmin%iter
    if(parmin%iter==0) parmin%epotitm1=f
    if(iproc==0 .and. parmin%iprint/=0) then
        fmax=calmaxforcecomponent(n,g)
        de1=f-parmin%epotitm1
        write(parmin%ifile,frt1) 'LBFGSMIN  ',parmin%iter,f,de1,gnorm,fmax,a_t
        write(*           ,frt1) 'LBFGSMIN  ',parmin%iter,f,de1,gnorm,fmax,a_t
    endif
    parmin%epotitm1=f
    if(iproc==0 .and. finish) then
        text=' the minimization terminated without detecting errors. iflag = 0'
        write(parmin%ifile,'(a)') text
        write(*           ,'(a)') text
    endif
end subroutine lb1_sp
!*****************************************************************************************
subroutine mcsrch_sp(n,x,f,g,s,a_t,info,nfev,wa,parmin) !line search routine mcsrch
    use minimization_sp, only:parameterminimization_sp
    type(parameterminimization_sp)::parmin
    integer::n,info,nfev
    real(8)::f,a_t
    ...
    do
        ...
        if(a_t==parmin%stpmin .and. (f>ftest1 .or. dg>=dgtest)) info=4
        ...
        if(stage1 .and. f<=fx .and. f>ftest1) then
            ...
            call mcstep_sp(a_l,fxm,dgxm,a_u,fym,dgym,a_t,fm,dgm, brackt,stmin,stmax,infoc,parmin)
            ...
            !call mcstep to update the interval of uncertainty and to compute the new step.
            call mcstep_sp(a_l,fx,dgx,a_u,fy,dgy,a_t,f,dg,brackt,stmin,stmax,infoc,parmin)
        end if
        ...
        yes=.true.
    enddo
end subroutine mcsrch_sp
!*****************************************************************************************
subroutine mcstep_sp(a_l,fx,dx,a_u,fy,dy,a_t,fp,dp,brackt,stpmin,stpmax,info,parmin)
    use minimization_sp, only:parameterminimization_sp
    type(parameterminimization_sp)::parmin
    integer::info
    ...
    if(fp>fx) then
        ...
        info = 1
        bound = .true.
        call cal_a_c_sp(a_l,fx,dx,a_t,fp,dp,a_c)
        ...
        info=2
        bound=.false.
        call cal_a_c_2_sp(a_l,fx,dx,a_t,fp,dp,a_c)
    ...
    elseif(abs(dp)<abs(dx)) then
        ...
        info=3
        bound=.true.
        call cal_a_c_3_sp(a_l,fx,dx,a_t,fp,dp,stpmin,stpmax,a_c)
        a_q=a_t+(dp/(dp-dx))*(a_l-a_t)
        if (brackt) then
    ...
    return
end subroutine mcstep_sp
!*****************************************************************************************
subroutine cal_a_c_sp(a_l,fx,dx,a_t,fp,dp,a_c)
    implicit none
...
end subroutine cal_a_c_sp
!*****************************************************************************************
subroutine cal_a_c_2_sp(a_l,fx,dx,a_t,fp,dp,a_c)
    implicit none
    real(8)::a_l,fx,dx,a_t,fp,dp,a_c,theta,s,gamma,r,q,p
    theta=3.d0*(fx-fp)/(a_t-a_l)+dx+dp
    s=max(abs(theta),abs(dx),abs(dp))
    ...
    r=p/q
    a_c=a_t+r*(a_l-a_t)
end subroutine cal_a_c_2_sp
!*****************************************************************************************
subroutine cal_a_c_3_sp(a_l,fx,dx,a_t,fp,dp,stpmin,stpmax,a_c)
    implicit none
