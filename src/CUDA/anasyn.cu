...
data ch  /  0.e0_wp            , -0.0033824159510050025955_wp, & 
  ...
  -0.0033824159510050025955_wp , 0.e0_wp /
   ...
   0.0018899503327676891843f
  -0.014952258337062199118f
   0.049137179673730286787f
  -0.051945838107881800736f
   0.77718575169962802862f
  -0.061273359067811077843f
   0.0076074873249766081919f
  -0.00054213233180001068935f
...
__global__ void waveletsynthesis(int n,int ndat, float *psi_in, float *psi_out,int idim)
{
  ...
  for(int i=0;i < par[idim].hwelem_calc[hwid]; ++i)
	...
	WTFIL0 *psi_sh[ShBaseElem               ] +
	WTFIL15*psi_sh[ShBaseElem + 15*NUM_LINES] +
	WTFIL1 *psi_sh[ShBaseElem +   NUM_LINES ] +
	WTFIL14*psi_sh[ShBaseElem + 14*NUM_LINES] +
	WTFIL2 *psi_sh[ShBaseElem + 2*NUM_LINES ] +
	WTFIL13*psi_sh[ShBaseElem + 13*NUM_LINES] +
	WTFIL3 *psi_sh[ShBaseElem + 3*NUM_LINES ] +
	WTFIL12*psi_sh[ShBaseElem + 12*NUM_LINES] +
	WTFIL4 *psi_sh[ShBaseElem + 4*NUM_LINES ] +
	WTFIL11*psi_sh[ShBaseElem + 11*NUM_LINES] +
	WTFIL5 *psi_sh[ShBaseElem + 5*NUM_LINES ] +
	WTFIL10*psi_sh[ShBaseElem + 10*NUM_LINES] +
	WTFIL6 *psi_sh[ShBaseElem + 6*NUM_LINES ] +
	WTFIL9 *psi_sh[ShBaseElem + 9*NUM_LINES ] +
	WTFIL7 *psi_sh[ShBaseElem + 7*NUM_LINES ] +
	WTFIL8 *psi_sh[ShBaseElem + 8*NUM_LINES ] ;
