/****u* CUDA/conv_pot_shared_kernel.cu
**
** 
** AUTHOR
**  Matthieu Ospici
...
__global__ void conv_pot_shared(unsigned int n1,unsigned int n2,float *t_out,float *t_in,int nf, float* pot)
{
  __shared__ float shared_temp[SIZE_SHARED_TOTAL];
  ...
  for(int i=0;i < TO_COPY_CALC;++i)
      ...
      for(unsigned int j=0 ;j < nf;++j)
	{
      ...
      t_out[(lineNumberFetch + SIZE_SHARED_1 *(bidx))*n2 +(i+offset_calc + baseOffset)] = tmp * pot[(lineNumberFetch + SIZE_SHARED_1 *(bidx))*n2 +(i+offset_calc + baseOffset)];
    }
