	...
	call deallocate_all
   	...
   	contains
		subroutine allocate_all
			...
			call memocc(i_stat,product(shape(kern_k1))*kind(kern_k1),'kern_k1','precong_per')
			allocate(kern_k2(0:n2),stat=i_stat)
			call memocc(i_stat,product(shape(kern_k2))*kind(kern_k2),'kern_k2','precong_per')
			allocate(kern_k3(0:n3),stat=i_stat)
			call memocc(i_stat,product(shape(kern_k3))*kind(kern_k3),'kern_k3','precong_per')
			allocate(z1(2,nd1b,nd2,nd3,2),stat=i_stat) ! work array for fft
			call memocc(i_stat,product(shape(z1))*kind(z1),'z1','precong_per')
			allocate(z3(2,nd1,nd2,nd3f,2),stat=i_stat) ! work array for fft
			call memocc(i_stat,product(shape(z3))*kind(z3),'z3','precong_per')
			allocate(x_c(0:n1,0:n2,0:n3),stat=i_stat)
			call memocc(i_stat,product(shape(x_c))*kind(x_c),'x_c','precong_per')
			
			allocate(x_f(7,nfl1:nfu1,nfl2:nfu2,nfl3:nfu3),stat=i_stat)
			call memocc(i_stat,product(shape(x_f))*kind(x_f),'x_f','precong_per')
			allocate(x_f1(nf),stat=i_stat)
			call memocc(i_stat,product(shape(x_f1))*kind(x_f1),'x_f1','precong_per')
			allocate(x_f2(nf),stat=i_stat)
			call memocc(i_stat,product(shape(x_f2))*kind(x_f2),'x_f2','precong_per')
			allocate(x_f3(nf),stat=i_stat)
			call memocc(i_stat,product(shape(x_f3))*kind(x_f3),'x_f3','precong_per')
			allocate(y_f(7,nfl1:nfu1,nfl2:nfu2,nfl3:nfu3),stat=i_stat)
			call memocc(i_stat,product(shape(y_f))*kind(y_f),'y_f','precong_per')
