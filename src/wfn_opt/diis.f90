...
!! SOURCE
!!
subroutine psimix(iproc,nproc,orbs,comms,diis,idsx,energy,energy_old,alpha,hpsit,psit)
  use module_base
  use module_types
  implicit none
  ...
  if (idsx > 0) then
     ...
     call diisstp(iproc,nproc,orbs,comms,diis,idsx,psit)
...
subroutine diis_or_sd(iproc,idsx,nkptsp,energy,idsx_actual,diis)
  use module_base
  use module_types
  implicit none
  ...
  type(diis_objects), intent(inout) :: diis
  integer, intent(out) :: idsx_actual

  !here we should add the DIIS/SD switch
  !add switch between DIIS and SD
  if (energy == diis%energy_min .and. .not. diis%switchSD) diis%idiistol=0
  if (energy > diis%energy_min .and. idsx >0 .and. .not. diis%switchSD) then
  ...
  if (diis%idiistol > idsx .and. .not. diis%switchSD) then
     ...
     if (iproc ==0) write(*,'(1x,a,1pe9.2,a)')&
          'WARNING: The energy value is growing (delta=',energy-diis%energy_min,') switch to SD'
     diis%switchSD=.true.
...
subroutine diisstp(iproc,nproc,orbs,comms,diis,idsx,psit)
  use module_base
  use module_types
  implicit none
! Arguments
  integer, intent(in) :: nproc,iproc,idsx
  type(orbitals_data), intent(in) :: orbs
  type(communications_arrays), intent(in) :: comms
  type(diis_objects), intent(inout) :: diis
...
!!     end do
  ...
  if (nproc > 1) then
     call mpiallred(rds(1,1),(idsx+1)*orbs%nkpts,MPI_SUM,MPI_COMM_WORLD,ierr)
...
!!                 MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
  ...
  do ikptp=1,orbs%nkptsp
     ...
     do i=1,min(diis%ids,idsx)
        diis%ads(i,min(idsx,diis%ids),ikptp,1)=rds(i,ikpt)
     end do

     ! copy to work array, right hand side, boundary elements
     do j=1,min(idsx,diis%ids)
        diis%ads(j,min(idsx,diis%ids)+1,ikptp,2)=1.0_wp
        rds(j,ikpt)=0.d0
        do i=j,min(idsx,diis%ids)
           diis%ads(j,i,ikptp,2)=diis%ads(j,i,ikptp,1)
        end do
     end do
     diis%ads(min(idsx,diis%ids)+1,min(idsx,diis%ids)+1,ikptp,2)=0.0_dp
     rds(min(idsx,diis%ids)+1,ikpt)=1.0_dp
     ...
     if (diis%ids > 1) then
        ...
        call DSYSV('U',min(idsx,diis%ids)+1,1,diis%ads(1,1,ikptp,2),idsx+1,  & 
             ipiv,rds(1,ikpt),idsx+1,diis%ads(1,1,ikptp,3),(idsx+1)**2,info)
...
! new guess
  ...
  if (iproc == 0 .and. verbose > 0) then 
     if (verbose < 10) then
        ...
        write(*,'(1x,a,2x,12(1x,1pe9.2))')'DIIS weights',(rds(j,1),j=1,min(idsx,diis%ids)+1)
     else
        do ikpt = 1, orbs%nkpts
           write(*,'(1x,a,I3.3,a,2x,12(1x,1pe9.2))')'DIIS weights (kpt #', ikpt, &
                & ')', (rds(j,0),j=1,min(idsx,diis%ids)+1)
        end do
     end if
  endif
