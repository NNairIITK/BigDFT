...
C	CALCULATES THE DISCRETE FOURIERTRANSFORM F(I1,I2)=
C	S_(j1,j2) EXP(isign*i*2*pi*(j1*i1/n1+j2*i2/n2)) R(j1,j2)
C	INPUT:
C	    n1,n2:physical dimension of the transform. It must be a 
C		  product of the prime factors 2,3,5, but greater than 3. 
C                 If two ni's are equal it is recommended to place them 
C                 behind each other.
C	    nd1,nd2:memory dimension of Z. ndi must always be greater or 
C		    equal than ni. It is recomended to chose ndi=ni 
C		    if ni is odd and ndi=ni+1 if ni is even to obtain 
C                   optimal execution speed. For small ni it can however 
C                   sometimes be advantageous to set ndi=ni
C           inzee=1: first part of Z is data array, second part work array
C           inzee=2: first part of Z is work array, second part data array
C		Z(1,i1,i2,inzee)=real(R(i1,i2))
C		Z(2,i1,i2,inzee)=imag(R(i1,i2))
C	OUTPUT:
C           inzee=1: first part of Z is data array, second part work array
C           inzee=2: first part of Z is work array, second part data array
C		real(F(i1,i2))=Z(1,i1,i2,inzee)
C		imag(F(i1,i2))=Z(2,i1,i2,inzee)
C            inzee on output is in general different from inzee on input
C	THE ARRAYELEMENTS Z( , , , ,3-inzee) ARE USED AS WORKING SPACE
C       On a RISC machine with cache, it is very important to find the optimal 
...
C	On a vector machine ncache has to be put to 0 
...
C       Commercial use is prohibited without the explicit permission of the author.
	...
	dimension trig(2,1024),zw(2,ncache/4,2),
     1	z(2,nd1*nd2,2),after(20),now(20),before(20)
	if (max(n1,n2).gt.1024) stop '1024'

c vector computer with memory banks:
      if (ncache.eq.0) then

C TRANSFORM ALONG Y AXIS
...
C	print*,'lot',lot
	...
	jj=ja*nd2-nd2+1
	nfft=jb-ja+1
	call fftrot(mm,nfft,m,mm,m,z(1,ja,inzee),z(1,jj,3-inzee),
     1	trig,after(i),now(i),before(i),isign)
	...
	i=1
	inzeep=2
	call fftstp(mm,nfft,m,nn,n,z(1,j,inzee),zw(1,1,3-inzeep),
     1	trig,after(i),now(i),before(i),isign)
	inzeep=1
...
2093	inzeep=3-inzeep
	...
	i=ic
	call fftrot(nn,nfft,n,mm,m,zw(1,1,inzeep),z(1,jj,3-inzee),
     1	trig,after(i),now(i),before(i),isign)
2000	continue
      endif
	inzee=3-inzee
...
C	print*,'lot',lot
	...
	jj=j*nd1-nd1+1
	nfft=jp-j+1
	call fftrot(mm,nfft,m,mm,m,z(1,j,inzee),z(1,jj,3-inzee),
     1	trig,after(i),now(i),before(i),isign)
