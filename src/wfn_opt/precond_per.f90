...
subroutine prec_fft(n1,n2,n3, &
  ...
  real*8,intent(in)::cprecr
 real*8,intent(in)::hgrid(3) 
  integer,intent(in):: keyg(2,nseg_c+nseg_f),keyv(nseg_c+nseg_f)
 
  real*8 hpsi(nvctr_c+7*nvctr_f)
real*8,allocatable,dimension(:)::kern_k1,kern_k2,kern_k3

 real*8,allocatable,dimension(:,:,:)::x_c,y_c! input and output of Fourier preconditioning
 real*8,allocatable::z(:,:,:,:,:) ! work array for FFT
 integer nd1,nd2,nd3
...
call wscal_f(nvctr_f,hpsi(nvctr_c+1),hgrid,cprecr)

call make_kernel(n1,hgrid(1),kern_k1)
call make_kernel(n2,hgrid(2),kern_k2)
call make_kernel(n3,hgrid(3),kern_k3)
...
subroutine wscal_f(mvctr_f,psi_f,hgrid,c)
! multiplies a wavefunction psi_c,psi_f (in vector form) with a scaling vector (scal)
implicit real(kind=8) (a-h,o-z)
real*8,intent(in)::c
dimension psi_f(7,mvctr_f),scal(7),hgrid(3),hh(3)
