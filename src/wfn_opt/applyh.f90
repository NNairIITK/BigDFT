   ...
   !!$     oidx=(iorb-1)*orbs%nspinor+1
     ...
     if (present(all_ham)) then ! lr408 hc
        if (all_ham /= 2) then
           ...
           call isf_to_daub_kinetic(hx,hy,hz,kx,ky,kz,orbs%nspinor,Lzd%Llr(ilr),wrk_lh(iilr),&
           ...
           call isf_to_daub(Lzd%Llr(ilr),wrk_lh(iilr),psir(1,1,iilr),hpsi(ispsi))!+nvctr*(ispinor-1)))
        end if
     else
        call isf_to_daub_kinetic(hx,hy,hz,kx,ky,kz,orbs%nspinor,Lzd%Llr(ilr),wrk_lh(iilr),&
             psir(1,1,iilr),hpsi(ispsi),ekin)
