...
contains
  ...
  subroutine initialize_OP2P_descriptors(symop,iproc,nproc,nobjects,objects_attributes,objects_repartition,OP2P)
    ...
    !assign the symmetry constraint to the structure
    OP2P%forsymop=symop
  ...
  subroutine wait_mpi(iproc,istep,nreq,requests)
    ...
    if (ierr /=0)  then
       write(*,*) 'ERROR WAITALL, iproc,step,ierr:',iproc,istep,ierr,mpistat
       ! Remove this to compile with XLF: ,MPI_STATUSES_IGNORE
    end if
