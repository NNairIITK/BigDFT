...
module module_input
   ...
   public :: input_var, input_dict_var
   public :: input_free
...
!!$         write(inout_lines(iline_written),"(A)") ")"
   ...
   ! New routines reading variables from 1-level dictionaries.
   !> Take a string.
...
!!$     end if
     ...
     if (present(exclusive)) then
        ...
        if (f_err_raise(.not. found, err_id = INPUT_VAR_NOT_IN_LIST, &
             & err_msg = trim(dict%data%key) // " = '" // trim(var) // "' is not allowed.")) return
     end if
   END SUBROUTINE dict_var_character
   ...
   subroutine dict_var_double(var,dict,default,ranges,exclusive)
     ...
     double precision, dimension(2), intent(in), optional :: ranges
     double precision, dimension(:), intent(in), optional :: exclusive
     !Local variables
...
!!$     end if
     ...
     !check the validity of the variable
     if (present(ranges)) then
        if (f_err_raise(var < ranges(1) .or. var > ranges(2), err_id = INPUT_VAR_NOT_IN_RANGE, &
             & err_msg = trim(dict%data%key) // " = '" // trim(val) // "' not in range [" // &
             & yaml_toa(ranges(1)) // ";" // yaml_toa(ranges(2)) // "].")) return
     else if (present(exclusive)) then
        found=.false.
        found_loop: do ilist=1,size(exclusive)
           if (var == exclusive(ilist)) then 
...
!!$     end if
   ...
   subroutine dict_var_int_arr(var, dict, default,ranges,exclusive)
     ...
     !Local variables
     integer :: i, ranges_(2)
     integer, dimension(:), allocatable :: exclusive_

     if (present(default)) then
        if (f_err_raise(size(var) /= size(default), err_id = INPUT_VAR_ARG_ERROR, &
             & err_msg = "inconsistency in array size for key '" // trim(dict%data%key) // "'.")) return
     end if

     ! Handling optional arguments.
     ranges_ = (/ -huge(1), huge(1) /)
     if (present(ranges)) ranges_ = ranges
     if (present(exclusive)) then
        allocate(exclusive_(size(exclusive)))
        exclusive_ = exclusive
     else
        allocate(exclusive_(0))
     end if
     if (present(default)) then
        do i = 1, size(var), 1
           call dict_var_int(var(i), dict // (i - 1), default(i), ranges_, exclusive_)
        end do
     else
        do i = 1, size(var), 1
           call dict_var_int(var(i), dict // (i - 1), ranges = ranges_, exclusive = exclusive_)
        end do
     end if
     deallocate(exclusive_)
   END SUBROUTINE dict_var_int_arr

   !> Take an double array
   subroutine dict_var_double_arr(var, dict, default,ranges,exclusive)
     ...
     type(dictionary), pointer :: dict
     double precision, dimension(:), intent(in), optional :: default
     double precision, dimension(2), intent(in), optional :: ranges
     double precision, dimension(:), intent(in), optional :: exclusive
     !Local variables
     integer :: i
     double precision :: ranges_(2)
     double precision, dimension(:), allocatable :: exclusive_
     ...
     if (present(default)) then
        do i = 1, size(var), 1
           call dict_var_double(var(i), dict // (i - 1), default(i), ranges_, exclusive_)
        end do
     else
        do i = 1, size(var), 1
           call dict_var_double(var(i), dict // (i - 1), ranges = ranges_,exclusive = exclusive_)
        end do
     end if
     deallocate(exclusive_)
   END SUBROUTINE dict_var_double_arr
