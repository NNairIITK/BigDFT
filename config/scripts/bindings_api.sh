...
for func in $GETS $API ; do
    ...
    file=`grep -nr --exclude=\*interfaces.f90 --exclude=\*unused\* --exclude=\*_fake.f90 --exclude=\*_old.f90 --exclude=\*private_api.f90 "^ *[sS][uU][bB][rR][oO][uU][tT][iI][nN][eE] *$func_case *(" $srcdir | cut -d':' -f1,2`
    echo "/* "$func $file" */"
