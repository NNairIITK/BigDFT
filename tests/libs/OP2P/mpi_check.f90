program mpi_check
  ...
  implicit none
  logical :: failed
  integer :: ntot,ierr,i,iproc,nproc
  ...
  allocate(buffer(ntot),copybuffer(ntot))
  ...
  call mpiallred(buffer(1),ntot,MPI_SUM,MPI_COMM_WORLD,ierr)
  failed=.false.
  do i=1,ntot
