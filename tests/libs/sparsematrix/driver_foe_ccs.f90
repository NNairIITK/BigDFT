program driver_css
  ...
  call sparse_matrix_and_matrices_init_from_file_ccs('overlap_ccs.dat', &
       bigdft_mpi%iproc, bigdft_mpi%nproc, bigdft_mpi%mpi_comm, smat_s, mat_s)
  call sparse_matrix_and_matrices_init_from_file_ccs('hamiltonian_ccs.dat', &
       bigdft_mpi%iproc, bigdft_mpi%nproc, bigdft_mpi%mpi_comm, smat_h, mat_h)
  ...
  charge(:) = 722.d0
  call init_foe(bigdft_mpi%iproc, bigdft_mpi%nproc, smat_s%nspin, charge, foe_obj)
  ...
  ! Write the result
  if (bigdft_mpi%iproc==0) call yaml_map('trace(KS) check',tr_KS_check)

  ! Write the difference to the previous result
  if (bigdft_mpi%iproc==0) call yaml_map('difference',tr_KS-tr_KS_check)
