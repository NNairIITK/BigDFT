program test_xc
  ...
  integer, dimension(n_funcs), parameter :: funcs = (/ &
       ...
       & 7, -1012, 8, 9, -1003, -1005, -1007, -1008, -1010, &
       & -1011, -1013, -1014, &
...
!!$          if (i == 1 .and. n == 1) call xc_dump()
  ...
  subroutine gauss(rho, n_rho, nspin)
    integer, intent(in) :: n_rho, nspin
    ...
    do j = 5, n_rho - 5
       rho(j, 1) = exp(-((sigma * (n_rho / 2 - j + delta)) ** 2)) / nspin / n_rho
       rho(j, 2) = exp(-((sigma * (n_rho / 2 - j - delta)) ** 2)) / nspin / n_rho
    end do
    call xc_clean_rho(n_rho * 2, rho, 1)
    rho = rho / sum(rho(:,1:nspin))
