...
!!!!>  !local variables
!!!!>  logical :: sum_pi,sum_ri,build_c,is_vextra,plot_cavity
...
!!!!>     call PS_gather(rhopot(i3start),kernel,dest=rhopot_full)
!!!!>     !call pkernel_build_epsilon(kernel,work_full,eps0,depsdrho,dsurfdrho)
!!!!>     call rebuild_cavity_from_rho(rhopot_full,nabla_rho,nabla2_rhopot,delta_rho,cc_rho,depsdrho,dsurfrho,&
...
!!!!>           call PS_gather(src=rhopot,dest=rhopot_full,kernel=kernel)
!!!!>           call polarization_charge(kernel%geocode,kernel%ndims,kernel%hgrids,kernel%nord,&
!!!!>                rho_full,pot,nabla_pot,rho_pol)
!!!!>        else
!!!!>           call polarization_charge(kernel%geocode,kernel%ndims,kernel%hgrids,kernel%nord,&
...
!!!!>
!!!!>
!!!!>  call PS_deallocate_lowlevel_workarrays(options%use_input_guess,&
!!!!>       rhopot(i3start),kernel)
!!!!>
...
!!!!>  case('VAC')
!!!!>     call finalize_hartree_results(present(pot_ion),pot_ion_eff,&
!!!!>          kernel%grid%m1,kernel%grid%m3,kernel%grid%n3p,&
...
!!!!>             rhopot(i3start),sum_pi,pot_ion_eff,vextra_eff,eVextra)
!!!!>       end if
!!!!>
!!!!>     !here the harteee energy can be calculated and the ionic potential
!!!!>     !added
!!!!>     call finalize_hartree_results(is_vextra,vextra_eff,& 
!!!!>          kernel%grid%m1,kernel%grid%m3,kernel%grid%n3p,&
!!!!>          kernel%grid%m1,kernel%grid%m3,kernel%grid%n3p,&
...
!!!!>        call add_Vextra(n1,n23,nabla2_pot(1,1,kernel%grid%istart+1),&
!!!!>             depsdrho,dsurfdrho,kernel%cavity,options%only_electrostatic,&
!!!!>             rhopot(i3start),sum_pi,pot_ion_eff,vextra_eff,eVextra)
...
!!!!>  end if
!!!!>
!!!!>  call PS_release_workarrays(xxx)
!!!!>
