subroutine pawpatch(energ,verbose,maxdim,pp,penal,&
  ...
  do LPaw=0, npawl-1     
     ...
     dum_energy=-2000.0D0
     do n=1,3
        aepot_cent=aepot + (Lpaw*(Lpaw+1) )/rgrid/rgrid   !! energies are in Rydeberg
        call difnrl(aepot_cent ,psi_initial(1,n) , dumpsi_p ,&
             Ngrid, a,b, rgrid,rgrid_ab,n+Lpaw ,Lpaw , znuc,dum_energy )
        print *, "L=", LPaw, " n=", n, " E= ", dum_energy
     end do
     deallocate(dumpsi_p)
     
     write(filename,'(a,I0)')'numerov_initial_L_',LPaw
