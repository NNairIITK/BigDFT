...
module dims
   implicit none
   integer, parameter :: norbmx=40, nrmax=10000, maxdim=30
   integer, parameter :: lmx=5, lpmx= 4, noccmx=7, nsmx=2
...
program pseudo
   ...
   do iline=1,999
      ...
      if(ierr<0)then
         ...
         if(iline==1)then
            ...
            write(6,*)
            write(6,'(2a)') '                   -ng      n ',  &
                 'number of gaussians - raise this for wider sigmamax'
            write(6,'(2a)') '                   -rij     f ',  &
                 'divider for gaussians - raise this for denser sigmamin' 
            write(6,*) '                  -denbas    ',  &
                 'dense basis with smaller steps simgan/simgan-1'
            write(6,*) '                  -mixref    ',  &
                 'allow mixed ae reference data, e.g. various rprb'
            write(6,*) '                  -plot      ',  &
            ...
            write(6,*)' keywords for fitting'
            write(6,*) '                  -cy      n ',  &
                 'number of fitting cycles (-1< n< 10000)'
            write(6,*) '                  -fit       ',  &
                 'equivalent to -cy 1'
            write(6,*) '                  -maxiter n ',  &
                 'max number of iterations per simplex cycle'
            write(6,*) '                  -inwidth f ',  &
                 'initial width of the random simplex'
            write(6,*) '                  -stuck   n ',  &
            ...
            write(6,*)' keywords for backwards compatibility:'
            write(6,*) '                  -orth      ',  &
            ...
            write(6,*)' keywords related to wavelets for softness:'
            write(6,*) '                  -nh      n ',  &
            ...
            write(6,*) '                  -offset  f ',  &
                 'offset between grid and atom'
            write(6,*)
            
            write(6,*)' keywords related to paw patches:'
            write(6,*) '                  -pawn       ',&
   ...
   nspin=1
   is(1) = '  so=0'
   if (methodae.eq.'r'.or.methodae.eq.'s') then
      nspin=2
      is(1)= 'so=+0.5'
      is(2)= 'so=-0.5'
   endif
   ...
   do iiter=1,max(namoeb,1)
      ...
      if (iiter.eq.1) then
         ...
         if(ierr/=0)then
            ...
            if(methodps.ne.'r'.and.methodps.ne.'n'.and.methodps.ne.'s')then
               ...
               write(6,*)
               write(6,*)'character found:',methodps
               write(6,*)'exiting.'
         ...
         is(1) = '  so=0'
         if (methodps.eq.'r'.or.methodps.eq.'s') then
            nspin=2
            is(1)= 'so=+0.5'
            is(2)= 'so=-0.5'
         endif
      ...
      if(iproc==0)then
         ...
         if(ipspcod==11)then
            ...
            write(13,'(2x,f16.8,3x, f16.8,6x,a)') & 
                 rcore, qcore, 'rcore, qcore (nlcc)'
            write( 6,'(2x,f16.8,3x, f16.8,6x,a)') & 
                 rcore, qcore, 'rcore, qcore (nlcc)'
         end if
         close(13)
         !        iproc is zero
