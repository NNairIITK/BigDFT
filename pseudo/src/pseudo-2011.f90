...
!! @author
!!    Alex Willand, under the supervision of Stefan Goedecker, July 2011
...
program pseudo
      ...
      dimension no(norbmx),noae(norbmx),lo(norbmx),so(norbmx),  &
           ...
           nomax(0:lmx),nomin(0:lmx),hso(6),havg(6), &
           time(3)

      character(len=1) :: il(5),ispp,isppp
      character(80):: errmsg
      character*7 is(2)
      character fname*35,tname*10,string*520
      character*80 label
...
!     Instead, read those options from a file input.dat (now input.pseudo)
...
! (TD 2013-09-03)
! Change the name weights.par into input.weights (from the last version of Alex Willand)

         write(6,*)
         write(6,*) 'Reading actual weights from file input.weights'
...
!               than some of the other processes, though.
                ...
                rprb=rprbp
                rcov=rcovp
!               ng and rij are taken from input.pseudo
