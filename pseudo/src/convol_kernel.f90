...
! submatrices are declared to be in CUDA shared memory
       ...
       integer*4, shared :: ibyz_fsub(0:0,0:7,1:2),lupfilsub
       real*8, shared :: filtersub(-14:14,1:4),psigsub(0:44,1:2,0:0,2:2,0:7,1:1)
...
ibyz_fsub(tx,ty,2) = ibyz_f(2,i,j)
call syncthreads()
do kb=ibyz_fsub(tx,ty,1),ibyz_fsub(tx,ty,2),blockdim%z
t112=0.d0;t121=0.d0;t122=0.d0;t212=0.d0;t221=0.d0;t222=0.d0;t211=0.d0
k = ibyz_fsub(tx,ty,2)-kb

!part of psig into shared memory (32+29)*1*8*2*8 B=7.808 KB

if (tz+kb-14 .ge. 0) then
  do ka=0,28,blockdim%z
   if (ka+tz .le. 28) then
    psigsub(ka+tz,1,tx,2,ty,1) = psig(ka+kb+tz-14,1,i,2,j,1)
    psigsub(ka+tz,2,tx,2,ty,1) = psig(ka+kb+tz-14,2,i,2,j,1)
   end if
  end do
  if (k .ge. blockdim%z) then
    psigsub(tz+29,1,tx,2,ty,1) = psig(kb+tz+15,1,i,2,j,1)
    psigsub(tz+29,2,tx,2,ty,1) = psig(kb+tz+15,2,i,2,j,1)
  else
   if (tz .le. k) then
    psigsub(tz+29,1,tx,2,ty,1) = psig(kb+tz+15,1,i,2,j,1)
    psigsub(tz+29,2,tx,2,ty,1) = psig(kb+tz+15,2,i,2,j,1)
...
if (nfu1-(kb+blockDim%z) .le. lupfilsub-1) then
 if (lhigh .le. lupfilsub-1) then
 do l=0,lhigh
 t121=t121+psigsub(tz+l+14,1,tx,2,ty,1)*filtersub(l,1)+psigsub(tz+l+14,2,tx,2,ty,1)*filtersub(l,2)
 t221=t221+psigsub(tz+l+14,1,tx,2,ty,1)*filtersub(l,3)+psigsub(tz+l+14,2,tx,2,ty,1)*filtersub(l,4)
 end do
 else
 do l=0,lupfilsub
 t121=t121+psigsub(tz+l+14,1,tx,2,ty,1)*filtersub(l,1)+psigsub(tz+l+14,2,tx,2,ty,1)*filtersub(l,2)
...
ekinvec(kb+tz,i,j)=ekinvec(kb+tz,i,j)+t121*psigsub(tz+14,1,tx,2,ty,1)+t221*psigsub(tz+14,2,tx,2,ty,1)
...
! submatrices are declared to be in CUDA shared memory
       ...
       integer*4, shared :: ibyz_fsub(0:0,0:7,1:2),lupfilsub
       real*8, shared :: filtersub(-14:14,4:4),psigsub(0:44,2:2,0:0,1:1,0:7,1:1)
...
!part of psig into shared memory (32+29)*1*8*2*8 B=3.904 KB

if (tz+kb-14 .ge. 0) then
  do ka=0,28,blockdim%z
   if (ka+tz .le. 28) then
    psigsub(ka+tz,2,tx,1,ty,1) = psig(ka+kb+tz-14,2,i,1,j,1)
   end if
  end do
  if (k .ge. blockdim%z) then
    psigsub(tz+29,2,tx,1,ty,1) = psig(kb+tz+15,2,i,1,j,1)
  else
   if (tz .le. k) then
    psigsub(tz+29,2,tx,1,ty,1) = psig(kb+tz+15,2,i,1,j,1)
   end if
...
if (nfu1-(kb+blockDim%z) .le. lupfilsub-1) then
 if (lhigh .le. lupfilsub-1) then
 do l=0,lhigh
 t211=t211+psigsub(tz+l+14,2,tx,1,ty,1)*filtersub(l,4)
 end do
 else
 do l=0,lupfilsub
 t211=t211+psigsub(tz+l+14,2,tx,1,ty,1)*filtersub(l,4)
...
ekinvec(kb+tz,i,j)=ekinvec(kb+tz,i,j)+t211*psigsub(tz+14,2,tx,1,ty,1)
...
! submatrices are declared to be in CUDA shared memory
       ...
       integer*4, shared :: ibxz_fsub(0:0,0:7,1:2),lupfilsub
       real*8, shared :: filtersub(-14:14,1:4),psigsub(0:0,2:2,0:44,1:2,0:7,1:1)
...
ibxz_fsub(tx,ty,2) = ibxz_f(2,i,j)
call syncthreads()
do kb=ibxz_fsub(tx,ty,1),ibxz_fsub(tx,ty,2),blockdim%z
t112=0.d0;t121=0.d0;t122=0.d0;t212=0.d0;t221=0.d0;t222=0.d0;t211=0.d0
k = ibxz_fsub(tx,ty,2)-kb
...
if (tz+kb-14 .ge. 0) then
  do ka=0,28,blockdim%z
   if (ka+tz .le. 28) then
    psigsub(tx,2,ka+tz,1,ty,1) = psig(i,2,ka+kb+tz-14,1,j,1)
    psigsub(tx,2,ka+tz,2,ty,1) = psig(i,2,ka+kb+tz-14,2,j,1)
   end if
  end do
  if (k .ge. blockdim%z) then
    psigsub(tx,2,tz+29,1,ty,1) = psig(i,2,kb+tz+15,1,j,1)
    psigsub(tx,2,tz+29,2,ty,1) = psig(i,2,kb+tz+15,2,j,1)
  else
   if (tz .le. k) then
    psigsub(tx,2,tz+29,1,ty,1) = psig(i,2,kb+tz+15,1,j,1)
    psigsub(tx,2,tz+29,2,ty,1) = psig(i,2,kb+tz+15,2,j,1)
...
if (nfu2-(kb+blockDim%z) .le. lupfilsub-1) then
 if (lhigh .le. lupfilsub-1) then
 do l=0,lhigh
  t211=t211+psigsub(tx,2,tz+l+14,1,ty,1)*filtersub(l,1)+psigsub(tx,2,tz+l+14,2,ty,1)*filtersub(l,2)
  t221=t221+psigsub(tx,2,tz+l+14,1,ty,1)*filtersub(l,3)+psigsub(tx,2,tz+l+14,2,ty,1)*filtersub(l,4)
 end do
 else
 do l=0,lupfilsub
  t211=t211+psigsub(tx,2,tz+l+14,1,ty,1)*filtersub(l,1)+psigsub(tx,2,tz+l+14,2,ty,1)*filtersub(l,2)
...
ekinvec(i,kb+tz,j)=ekinvec(i,kb+tz,j)+t211*psigsub(tx,2,tz+14,1,ty,1)+t221*psigsub(tx,2,tz+14,2,ty,1)
...
! submatrices are declared to be in CUDA shared memory
       ...
       integer*4, shared :: ibxz_fsub(0:0,0:7,1:2),lupfilsub
       real*8, shared :: filtersub(-14:14,4:4),psigsub(0:0,1:1,0:44,2:2,0:7,1:1)
...
if (tz+kb-14 .ge. 0) then
  do ka=0,28,blockdim%z
   if (ka+tz .le. 28) then
    psigsub(tx,1,ka+tz,2,ty,1) = psig(i,1,ka+kb+tz-14,2,j,1)
   end if
  end do
  if (k .ge. blockdim%z) then
    psigsub(tx,1,tz+29,2,ty,1) = psig(i,1,kb+tz+15,2,j,1)
  else
   if (tz .le. k) then
    psigsub(tx,1,tz+29,2,ty,1) = psig(i,1,kb+tz+15,2,j,1)
   end if
...
if (nfu2-(kb+blockDim%z) .le. lupfilsub-1) then
 if (lhigh .le. lupfilsub-1) then
 do l=0,lhigh
  t121=t121+psigsub(tx,1,tz+l+14,2,ty,1)*filtersub(l,4)
 end do
 else
 do l=0,lupfilsub
  t121=t121+psigsub(tx,1,tz+l+14,2,ty,1)*filtersub(l,4)
...
ekinvec(i,kb+tz,j)=ekinvec(i,kb+tz,j)+t121*psigsub(tx,1,tz+14,2,ty,1)
...
! submatrices are declared to be in CUDA shared memory
       ...
       integer*4, shared :: ibxy_fsub(0:0,0:7,1:2),lupfilsub
       real*8, shared :: filtersub(-14:14,1:4),psigsub(0:0,1:1,0:7,2:2,0:44,1:2)
...
ibxy_fsub(tx,ty,2) = ibxy_f(2,i,j)
call syncthreads()
do kb=ibxy_fsub(tx,ty,1),ibxy_fsub(tx,ty,2),blockdim%z
t112=0.d0;t121=0.d0;t122=0.d0;t212=0.d0;t221=0.d0;t222=0.d0;t211=0.d0
...
if (tz+kb-14 .ge. 0) then
  do ka=0,28,blockdim%z
   if (ka+tz .le. 28) then
    psigsub(tx,1,ty,2,ka+tz,1) = psig(i,1,j,2,ka+kb+tz-14,1)
    psigsub(tx,1,ty,2,ka+tz,2) = psig(i,1,j,2,ka+kb+tz-14,2)
   end if
  end do
  if (k .ge. blockdim%z) then
    psigsub(tx,1,ty,2,tz+29,1) = psig(i,1,j,2,kb+tz+15,1)
    psigsub(tx,1,ty,2,tz+29,2) = psig(i,1,j,2,kb+tz+15,2)
  else
   if (tz .le. k) then
    psigsub(tx,1,ty,2,tz+29,1) = psig(i,1,j,2,kb+tz+15,1)
    psigsub(tx,1,ty,2,tz+29,2) = psig(i,1,j,2,kb+tz+15,2)
...
if (nfu3-(kb+blockDim%z) .le. lupfilsub-1) then
 if (lhigh .le. lupfilsub-1) then
 do l=0,lhigh
  t121=t121+psigsub(tx,1,ty,2,tz+l+14,1)*filtersub(l,1)+psigsub(tx,1,ty,2,tz+l+14,2)*filtersub(l,2)
  t122=t122+psigsub(tx,1,ty,2,tz+l+14,1)*filtersub(l,3)+psigsub(tx,1,ty,2,tz+l+14,2)*filtersub(l,4)
 end do
 else
 do l=0,lupfilsub
  t121=t121+psigsub(tx,1,ty,2,tz+l+14,1)*filtersub(l,1)+psigsub(tx,1,ty,2,tz+l+14,2)*filtersub(l,2)
...
ekinvec(i,j,kb+tz)=ekinvec(i,j,kb+tz)+t121*psigsub(tx,1,ty,2,tz+14,1)+t122*psigsub(tx,1,ty,2,tz+14,2)
...
! submatrices are declared to be in CUDA shared memory
       ...
       integer*4, shared :: ibxy_fsub(0:0,0:7,1:2),lupfilsub
       real*8, shared :: filtersub(-14:14,4:4),psigsub(0:0,1:1,0:7,1:1,0:44,1:2)
...
if (tz+kb-14 .ge. 0) then
  do ka=0,28,blockdim%z
   if (ka+tz .le. 28) then
    psigsub(tx,1,ty,1,ka+tz,2) = psig(i,1,j,1,ka+kb+tz-14,2)
   end if
  end do
  if (k .ge. blockdim%z) then
    psigsub(tx,1,ty,1,tz+29,2) = psig(i,1,j,1,kb+tz+15,2)
  else
   if (tz .le. k) then
    psigsub(tx,1,ty,1,tz+29,2) = psig(i,1,j,1,kb+tz+15,2)
   end if
...
if (nfl2-kb+14 .ge. 1) then
 if (llow .ge. 1) then
 do l=llow,13
  t121=t121+psigsub(tx,1,ty,1,tz+l,2)*filtersub(-14+l,4)
 end do
 else
 do l=0,13
  t121=t121+psigsub(tx,1,ty,1,tz+l,2)*filtersub(-14+l,4)
...
if (nfu2-(kb+blockDim%z) .le. lupfilsub-1) then
 if (lhigh .le. lupfilsub-1) then
 do l=0,lhigh
  t121=t121+psigsub(tx,1,ty,1,tz+l+14,2)*filtersub(l,4)
 end do
 else
 do l=0,lupfilsub
  t121=t121+psigsub(tx,1,ty,1,tz+l+14,2)*filtersub(l,4)
...
ekinvec(i,j,kb+tz)=ekinvec(i,j,kb+tz)+t121*psigsub(tx,1,ty,1,tz+14,2)

y(i,1,j,1,kb+tz,2)=y(i,1,j,1,kb+tz,2)+t121
call syncthreads()
