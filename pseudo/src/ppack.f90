...
!      print*,'nfit=',nfit
      ...
      if (nstring.eq.'init') then
         ...
         open(20,file='FITPAR')
!        Let us parse the FITPAR file line by line
         do iline=1,999
...
!                errors for reading strings should not happen
                 write(6,*)'Reading error for file FITPAR'
                 exit
              end if

!             "auto", automatic assignment requested:
              if( index(string,'auto')>0)then
                 if(verbose) write(6,*)'Automatic assignment'
...
!                r_l2 are freed if they differ from r_l.
!                rcore is disabled when negative.
                 lpack(6) = ( rcore > 0d0)
                 do i=1,4
...
!        print some help if nothing matches
         if(.not.any(lpack) .and. verbose)then
            ...
            write(6,*)'fitting variables in the file FITPAR.'
            write(6,*)
...
!           this warning message does not need the error handler
            write(6,*)
            write(6,*)'                WARNING' 
            write(6,*)'In FITPAR there are free parameters for the'
            ...
            write(6,'(2(a,i3))')'psppar:',lpx,', FITPAR:',lpxfit
            write(6,*)
            lpx = lpxfit
         endif
...
!
!     index offset from new FITPAR for core (5) and r_l2 (one per l)
