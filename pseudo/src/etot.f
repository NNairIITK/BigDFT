...
c     The computation of the enerrgy terms is clumsy and inefficient.
c     Need to improve this after proof of concept that it works.
...
c     calc Exc and Vxc first, taking into account a possible NLCC.
c     rhogrd contains the core charge at this point in case of a NLCC.
c     note: vxcgrd as returned by ggaenergy was already multifplied by rw/4pi in gatom
c     use the same loop to strip off the core charge 
c     and to add up the charges from  both channels.
c     We do not need polarization nor core charge for the other energy terms!
      do ispol=1,nspol
      do k=1,nint
...
c     first and possibly only spin channel:
      call DGEMV('N',nint,((ng+1)*(ng+2))/2*(lcx+1),1.d0,ud,nint,
     &             rho(:,:,1),1,0.d0,vhgrd,1)
CCCc                          ^    ^
c    NOTE: This is somewhat nonsense. Call DGEMV once with the sum of rho
CCCc     add the second charge channel in the polarized case
CCCc     calling DGEMV twice is inefficient, but let us just try that for now.
CCC      if(nspol==2)  
CCC     &call DGEMV('N',nint,((ng+1)*(ng+2))/2*(lcx+1),1.d0,ud,nint,
CCC     &             rho(:,:,2),1,1.d0,vhgrd,1)
c                          ^    ^


      

c   calc eext and ehart
      do k=1,nint
         r=rr(k)
     ...
     1        gpot(4)*(r/rloc)**6 )
c        write(88,*)rr(k),vhgrd(k),rhogrd(k,1)+rhogrd(k,2),rw(k)
         eext  = eext  +      vexgrd(k)*rhogrd(k,1)*rw(k)
         ehart = ehart + 0.5d0*vhgrd(k)*rhogrd(k,1)*rw(k)

      enddo

c     calc the kinetic energy term and the separable part of the pseudopotential
...
c     kinetic energy	
     ...
     :                       *exp(-xp(i)*r**2))
                     enddo
c                    below line seems to be nonsense!
c                    rkin=rkin*r**ll
...
c     separabel part
     ...
     :                       *exp(-.5d0*(r/r_l(ll+1))**2)
                  ...
                  enddo
c             write(*,'(a,f20.12,3i3,f10.3)')
c    :       'DEBUG: ekin component',(ekin-dd)/zz,ll,ispin,iocc,zz
               endif
            enddo
         enddo
      enddo
...
c        if(nspol==2)then
c        do k=1,nint
c           denfull=denfull+rhogrd(k,2)*rw(k)
c        enddo
c        end if
         if(rcore>0d0)denful=denful-znuc+zion
