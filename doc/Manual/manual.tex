...
atom one draws a sphere whose radii are the 'size of the atom' times the parameter {\bf crmult}. All the grid 
...
'angstroem' or 'angstroemd0' if the angstr\"om unit is used. 'd0' formats (e24.17) guarantee that not a single bit is 
...
\item {\bf ncong}, {\bf idsx}: ncong gives the number of iterations in the solution of the preconditioning equation.
      ...
      the wavefunction optimization, but each iteration is more costly. So an optimal compromise value has to be found.
...
\item {\bf calc\_tail}, {\bf rbuf}, {\bf ncongt}:  Far reaching tails of the wavefunctions decaying into the vacuum are  
                                      added in a perturbative treatment if the variable {\bf calc\_tail} is set to \texttt{.TRUE.}. 
