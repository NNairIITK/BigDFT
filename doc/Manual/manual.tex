...
once-and-for-all strategy for calcuylating the PSP projectors, which
...
caculations, and surface boundary conditions also.
\end{itemize}
...
\item {\bf geopt\_approach}: This character string specifies the method used for the geometry optimization.
       ...
       \item  \emph{BFGS}: Limited Memory BFGS;
...
are present:
\begin{itemize}
  \item \textbf{band keyword}: a character string containing
\emph{bands}.
...
\begin{itemize}
  ...
  \item  \textbf{'fftcache' ncache\_size}: Specify the cache sier for FFT in kBytes.
\end{itemize}
...
Using the excutable \texttt{frequencies}, it is possible to calculate the vibrational properties of
...
1 & LDA or LSD, Teter Pade parametrization\\
  & (published in S. Goedecker, M. Teter, J. H\"utter,cPhys. Rev. B54, 1703 (1996)),\\
