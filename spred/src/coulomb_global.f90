...
!  real(gp), parameter :: bohr=0.5291772108_gp !1 AU in angstroem
  ...
  real(kind=8) :: accepted,ediff,q_amp,gauss_width,av_q_amp,av_ediff,escape,escape_sam
  ...
  real(kind=8), dimension(:,:), pointer :: rxyz_opt,rxyz_coulomb
  
  type(run_objects) :: run_opt,run_coulomb !< the two runs parameters
  ...
  call bigdft_set_run_properties(run,input_id='coulomb'//trim(run_id), log_to_disk=.false.)

  call run_objects_init(run_coulomb,run,source=run_opt)
  
  !options and run are not needed
  call dict_free(options)
  nullify(run)

  !at this point we have run_opt and run_coulomb that are 
...
!   write(*,*) 'nat=',atoms%astruct%nat
  ...
  !performs few checks
  if (run_opt%inputs%inguess_geopt .ne. run_coulomb%inputs%inguess_geopt) then 
     write(*,*) "input guess methods in coulomb and OPT have to be identical"
     stop
  endif

  !associate the same output directory 
  if (run_opt%inputs%dir_output /= run_coulomb%inputs%dir_output) then
     if (bigdft_mpi%iproc == 0) then
        call deldir(trim(run_coulomb%inputs%dir_output),len_trim(run_coulomb%inputs%dir_output),ierr)
        if (ierr /=0) then
           call yaml_warning('Error found while deleting '//&
                trim(run_coulomb%inputs%dir_output))
        end if
     end if
     run_coulomb%inputs%dir_output=run_opt%inputs%dir_output
  end if
  ...
  rxyz_coulomb => bigdft_get_rxyz_ptr(run_coulomb)
...
! read random offset
  ...
  if (bigdft_mpi%iproc==0) then 
     ...
     open(unit=2,file=trim(run_coulomb%inputs%dir_output)//'global.mon',status='unknown',position='append')
  ...
  write(filename,'(a6,i3.3)') 'coulomb_ioput'//trim(naming_id) !,bigdft_mpi%iproc
  open(unit=11,file='coulomb_ioput'//trim(naming_id),status='old') 
  read(11,*) ediff,q_amp,gauss_width
  close(11)
  if (bigdft_mpi%iproc == 0) call yaml_map('(MH) Input ediff, q_amp, gauss_width',(/ediff,q_amp,gauss_width/),fmt='(1pe10.3)')
  
  call reset_coulomb_force_count()
  call alloc_dim_charges_arr(natoms)
  call set_coulomb_gauss_width(gauss_width)
  call set_q_amp(q_amp)
...
!C first local minimum
  ...
  call bigdft_get_input_policy(run_coulomb, run_policy)
  if (run_policy/=INPUT_POLICY_SCRATCH) then
      call f_err_throw('Wrong run policy ('//trim(yaml_toa(run_policy))//'), only '&
          //trim(yaml_toa(INPUT_POLICY_SCRATCH,fmt='(i0)'))//' is possible',&
          err_name='BIGDFT_RUNTIME_ERROR')
  ...
  !we start 
  call bigdft_state(run_coulomb,outs,infocode)
...
!     write(*,*) 'filename: ',filename
  ...
  call geopt(run_coulomb, outs, bigdft_mpi%nproc,bigdft_mpi%iproc,ncount_bigdft)
...
!      close(864)
!  endif
! call yaml_map('Conditions for ha_trans',[bigdft_get_geocode(run_coulomb)=='F',(.not. disable_hatrans)])


  if (bigdft_get_geocode(run_coulomb)=='F' .and. (.not. disable_hatrans)) call ha_trans(bigdft_nat(run_coulomb),rxyz_coulomb)
...
!  endif   
  ...
  call bigdft_set_rxyz(run_opt,rxyz=rxyz_coulomb)!bigdft_get_rxyz_ptr(run_coulomb))
...
!!$     enddo
  ...
  if (bigdft_mpi%iproc == 0) then
          ...
          escape,e_pos,ediff,get_q_amp(),'  P ',nvisit 
...
555 continue
  ...
  call bigdft_set_rxyz(run_coulomb,rxyz=pos) !one could write here also rxyz=bigdft_get_rxyz_ptr(run_opt)
  escape=escape+1.d0
  
  run_coulomb%add_coulomb_force = .true.
  call distribute_random_charges(natoms)
  call geopt(run_coulomb, outs, bigdft_mpi%nproc,bigdft_mpi%iproc,ncount_bigdft)
  run_coulomb%add_coulomb_force = .false.
...
!!     call bigdft_write_atomic_file(run_coulomb,outs,&
...
!!$          outs%energy,atoms%astruct%rxyz,atoms%astruct%ixyz_int,atoms,trim(comment),forces=outs%fxyz)
     ...
     if (bigdft_get_geocode(run_coulomb) == 'F') &
          & call fixfrag_posvel(bigdft_mpi%iproc,bigdft_nat(run_coulomb),rcov,rxyz_coulomb,vxyz,1,occured)
     if (bigdft_get_geocode(run_coulomb) == 'S') &
          & call fixfrag_posvel_slab(bigdft_mpi%iproc,bigdft_nat(run_coulomb),rcov,rxyz_coulomb,vxyz,1)
     
  av_q_amp=av_q_amp + get_q_amp()
  ncount_bigdft=0
...
!  endif   
  !call run_objects_associate(runObj, inputs_opt, atoms, rst)
  call bigdft_set_rxyz(run_opt,rxyz=rxyz_coulomb)
...
call fingerprint(spredinputs,nid,bigdft_nat(run_opt),bigdft_get_cell(run_opt),rcov,rxyz_opt,wfp)
     ...
     if (d.lt.fp_delta) then ! not escaped
       escape_sam=escape_sam+1.d0
        fp_sep=max(fp_sep,d)
        call set_q_amp(q_amp*beta_S)
        if (bigdft_mpi%iproc == 0) then
             q_amp = get_q_amp()
             call coulomb_wtioput(naming_id,ediff,q_amp,gauss_width)
             
             write(2,'((1x,f10.0),1x,1pe21.14,2(1x,1pe10.3),3(1x,0pf5.2),a)')  &
             escape,outs%energy,ediff,q_amp, &
  ...
  if (newmin) then
      ...
      if (bigdft_mpi%iproc == 0) then
         ...
         !yaml_comment '(MH) COULOMB_WINTER' needs to be made : Abhishek
         q_amp = get_q_amp()
         call coulomb_winter(naming_id,natoms,bigdft_get_astruct_ptr(run_opt),&
                                 nid,nlminx,nlmin,singlestep,en_delta,fp_delta,en_arr,ct_arr,&
                                 fp_arr,pl_arr,ediff,q_amp,gauss_width)
        endif
...
!!$       call write_atomic_file('poscur'//trim(bigdft_run_id_toa()),e_pos,pos,atoms%astruct%ixyz_int,atoms,'')
     ...
     q_amp = get_q_amp()
     if (bigdft_mpi%iproc == 0) call coulomb_wtioput(naming_id,ediff,q_amp,gauss_width)
...
!3000 continue
  ...
  if (bigdft_mpi%iproc == 0) then
     ...
     q_amp = get_q_amp()
     call coulomb_winter(naming_id,natoms,bigdft_get_astruct_ptr(run_opt),nid,nlminx,nlmin,singlestep,en_delta,fp_delta, &
           en_arr,ct_arr,fp_arr,pl_arr,ediff,q_amp,gauss_width)
  endif
  ...
  if (bigdft_mpi%iproc == 0) then
     ...
     call yaml_map('(MH) average q_amp',av_q_amp/escape)
     ...
     q_amp = get_q_amp()
     call yaml_map('(MH) q_amp out',q_amp)
     call yaml_map('(MH) gauss_width out',gauss_width)
     call yaml_mapping_close()
  endif
  close(2) 
...
!  if (iproc==0) write(*,*) 'quit 1'
  call release_run_objects(run_coulomb)
...
subroutine coulomb_winter(naming_id,nat,astruct,nid,nlminx,nlmin,singlestep,en_delta,fp_delta, &
     en_arr,ct_arr,fp_arr,pl_arr,ediff,q_amp,gauss_width)
  use module_base
...
!!$  use m_ab6_symmetry
  ...
  integer, intent(in) :: nlminx,nlmin,nid
  real(gp), intent(in) :: ediff,q_amp,gauss_width,en_delta,fp_delta
...
END SUBROUTINE coulomb_winter


subroutine coulomb_wtioput(naming_id,ediff,q_amp,gauss_width)
  implicit none
  !Arguments
  character(len=*), intent(in) :: naming_id
  real(kind=8), intent(in) :: ediff,q_amp,gauss_width
  !Local variables
  integer, parameter :: iunit=11
  open(unit=iunit,file='coulomb_ioput'//trim(naming_id),status='unknown')
  write(iunit,'(3(1x,1pe24.17)1x,a)') ediff,q_amp,gauss_width,' ediff, q_amp, gauss_width'
  close(unit=iunit)
END SUBROUTINE coulomb_wtioput
